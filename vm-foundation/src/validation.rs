...
use crate::error::{Architecture, GuestAddr, RegId};
use crate::error::{ErrorContext, VmError, VmResult};
use regex::Regex;
    ...
    pub fn to_result(self) -> VmResult<()> {
        if self.is_valid {
            ...
            Err(VmError::Configuration {
                source: crate::error::ConfigError::ValidationFailed(
                    self.errors
                        .iter()
                        .map(|e| e.to_string())
                        .collect::<Vec<_>>()
...
pub mod utils {
    ...
    pub fn validation_error(
        ...
        VmError::Configuration {
            source: crate::error::ConfigError::ValidationFailed(format!(
                "{}: {}",
                field_name,
                message.into()
            )),
