...
impl Serialize for ExecutionStats {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field(
            "last_update_time_ns",
            &self.last_update_time.map(|i| i.elapsed().as_nanos() as u64),
        )?;
        state.end()
    }
}
    ...
    pub fn record_allocation(&mut self, size: u64) {
        ...
        if self.total_memory_bytes > 0 {
            self.fragmentation_ratio =
                1.0 - (self.allocated_memory_bytes as f64 / self.total_memory_bytes as f64);
        }
    }
...
impl Stats for MemoryStats {
    ...
    fn merge(&mut self, other: &Self) {
        ...
        if total_allocations > 0 {
            self.fragmentation_ratio = (self.fragmentation_ratio * self.allocation_count as f64
