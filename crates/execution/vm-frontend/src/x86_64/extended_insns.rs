...
impl ExtendedDecoder {
    ...
    pub fn decode_avx_insn(opcode: &[u8], _pc: GuestAddr) -> Option<AvxInstruction> {
        ...
        if is_vex2 && opcode.len() >= 3 {
            ...
            let _is_256 = (vex_byte & 0x04) != 0; // L bit

            match opcode_byte {
                0x28 => Some(AvxInstruction::Vmovaps),
                0x58 => Some(AvxInstruction::Vaddps),
                0x59 => Some(AvxInstruction::Vmulps),
                0x5C => Some(AvxInstruction::Vsubps),
                0x5E => Some(AvxInstruction::Vdivps),
                0x51 => Some(AvxInstruction::Vsqrtps),
                0x5F => Some(AvxInstruction::Vmaxps),
                0x5D => Some(AvxInstruction::Vminps),
                _ => None,
            }
        } else if is_vex3 && opcode.len() >= 4 {
            let opcode_byte = opcode[3];
            let _is_256 = (opcode[2] & 0x04) != 0; // L bit in second VEX byte
...
mod tests {
    ...
    fn test_sse_decode() {
        ...
        let insn = ExtendedDecoder::decode_sse(&opcode, GuestAddr(0x1000));
        assert!(insn.is_some());
    }
    ...
    fn test_avx_decode() {
        ...
        let insn = ExtendedDecoder::decode_avx(&opcode, GuestAddr(0x1000));
        assert!(insn.is_some());
    }
}
