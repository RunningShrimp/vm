...
//!
//! 提供统一的硬件直通管理接口
//! 整合PCIe、GPU、NPU、SR-IOV等功能
//! 从 vm-passthrough 模块迁移而来
...
impl std::fmt::Display for PassthroughError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PassthroughError::InvalidAddress(s) => write!(f, "Invalid PCI address: {}", s),
            PassthroughError::DeviceNotFound(s) => write!(f, "Device not found: {}", s),
            PassthroughError::DeviceInUse => write!(f, "Device already in use"),
            PassthroughError::PermissionDenied => write!(f, "Permission denied"),
            PassthroughError::IommuNotEnabled => write!(f, "IOMMU not enabled"),
            PassthroughError::DriverBindingFailed(s) => write!(f, "Driver binding failed: {}", s),
            PassthroughError::IoError(s) => write!(f, "IO error: {}", s),
        }
    }
}
...
impl From<std::io::Error> for PassthroughError {
    fn from(err: std::io::Error) -> Self {
        PassthroughError::IoError(err.to_string())
    }
}
