...
impl PerformanceAnalyzer {
    ...
    pub async fn analyze(&self) -> PerformanceAnalysis {
        ...
        let recommendations = self
            .generate_recommendations(&bottlenecks, &current_metrics)
            .await;
        ...
        let overall_score = self
            .calculate_overall_score(&current_metrics, &bottlenecks)
            .await;
    ...
    async fn identify_bottlenecks(&self, metrics: &SystemMetrics) -> Vec<Bottleneck> {
        ...
        // 检查编译时间
        if metrics.jit_metrics.avg_compile_time_ns > 10_000_000.0 {
            // 10ms
        ...
        // 检查执行时间
        if metrics.jit_metrics.avg_execution_time_ns > 1_000_000.0 {
            // 1ms
            bottlenecks.push(Bottleneck {
        ...
        // 检查GC时间
        if metrics.gc_metrics.avg_gc_time_ns > 50_000_000.0 {
            // 50ms
            bottlenecks.push(Bottleneck {
            ...
            match bottleneck.category {
                BottleneckCategory::Compilation => {
                    recommendations.push(Recommendation {
                        ...
                        description: "考虑启用异步编译、增加编译缓存大小、优化编译器配置"
                            .to_string(),
                ...
                BottleneckCategory::Execution => {
                    recommendations.push(Recommendation {
                        ...
                        description: "检查热点代码优化、考虑启用SIMD优化、优化寄存器分配"
                            .to_string(),
        ...
        if historical.len() > 1 {
            ...
            analysis.insert(
                ...
                MetricAnalysis {
                    ...
                    is_anomaly: current.jit_metrics.avg_compile_time_ns
        ...
        if historical.len() > 1 {
            ...
            analysis.insert(
                ...
                MetricAnalysis {
                    ...
                    is_anomaly: current.jit_metrics.avg_execution_time_ns
                        > self.percentile(&sorted, 0.95),
                },
            );
        }
    ...
    async fn calculate_trends(&self, historical: &[SystemMetrics]) -> Vec<Trend> {
        ...
        if compile_times.len() >= 2 {
            ...
            let change_percent = if compile_times[0] > 0.0 {
                (compile_times[compile_times.len() - 1] - compile_times[0]) / compile_times[0]
                    * 100.0
            } else {
                0.0
            };
