...
use crate::{CoreError, ExecMode, GuestArch, VmConfig, VmError};

/// 配置验证错误
#[derive(Debug, Error, Clone)]
pub enum ConfigValidationError {
    /// 内存大小无效
    #[error("Invalid memory size: {} MB (min: {} MB, max: {} MB)",
            size / (1024 * 1024),
            min / (1024 * 1024),
            max / (1024 * 1024))]
    InvalidMemorySize { size: usize, min: usize, max: usize },
    /// vCPU数量无效
    #[error("Invalid vCPU count: {} (min: {}, max: {})", count, min, max)]
    InvalidVcpuCount {
        count: usize,
        min: usize,
        max: usize,
    ...
    /// 执行模式与架构不兼容
    #[error("Execution mode {:?} incompatible with architecture {:?}", mode, arch)]
...
impl From<ConfigValidationError> for VmError {
    fn from(err: ConfigValidationError) -> Self {
        match &err {
            ConfigValidationError::InvalidMemorySize { size, min, max } => {
                VmError::Core(CoreError::InvalidConfig {
                    message: format!(
                        "memory_size {} MB not in range [{} MB, {} MB]",
                        size / (1024 * 1024),
                        min / (1024 * 1024),
                        max / (1024 * 1024)
                    ),
            ...
            ConfigValidationError::InvalidVcpuCount { count, min, max } => {
                VmError::Core(CoreError::InvalidConfig {
                    message: format!("vcpu_count {} not in range [{}, {}]", count, min, max),
                    field: "vcpu_count".to_string(),
                })
            }
            ConfigValidationError::UnsupportedArchitecture { arch } => {
                VmError::Core(CoreError::NotSupported {
                    feature: format!("Architecture {:?}", arch),
                    module: "vm-interface::config_validator".to_string(),
                })
            }
            ConfigValidationError::IncompatibleExecMode { mode, arch } => {
                VmError::Core(CoreError::InvalidConfig {
                    message: format!(
                        "Execution mode {:?} incompatible with architecture {:?}",
                        mode, arch
                    ),
                    field: "exec_mode".to_string(),
                })
            }
            ConfigValidationError::ConfigConflict {
                ...
                message: format!("Conflict: '{}' vs '{}': {}", field1, field2, reason),
                path: Some(format!("{}.{}", field1, field2)),
            }),
            ...
            ConfigValidationError::InvalidFilePath { path, reason } => {
                VmError::Core(CoreError::InvalidConfig {
...
mod tests {
    ...
    #[test]
    fn test_config_validation_error_to_vm_error_conversion() {
        use crate::VmError;

        // Test InvalidMemorySize conversion
        let err = ConfigValidationError::InvalidMemorySize {
            size: 512,
            min: 1024 * 1024,
        ...
        // Test InvalidVcpuCount conversion
        let err = ConfigValidationError::InvalidVcpuCount {
            count: 0,
            min: 1,
            max: 256,
        };
        ...
        let err = ConfigValidationError::UnsupportedArchitecture {
            arch: crate::GuestArch::X86_64,
        };
        let vm_err: VmError = err.into();
        assert!(matches!(vm_err, VmError::Core(_)));

        // Test ConfigConflict conversion
        let err = ConfigValidationError::ConfigConflict {
            field1: "memory_size".to_string(),
            field2: "max_memory".to_string(),
