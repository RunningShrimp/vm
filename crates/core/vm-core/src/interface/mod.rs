...
use crate::{ExecResult, ExecStats, GuestAddr, VmError};

// 使用类型别名避免循环依赖，实际类型由实现决定
// IRBlock 将在使用时通过 ExecutionEngine<vm_ir::IRBlock> 具体化
...
pub trait ExecutionEngine<I>: VmComponent + Configurable + Observable {
    ...
    /// 执行IR块
    fn execute<M: super::mmu_traits::MMU>(&mut self, mmu: &mut M, block: &I) -> ExecResult;
    ...
    /// 异步执行版本
    fn execute_async<M: super::mmu_traits::MMU>(
        &mut self,
        mmu: &mut M,
        block: &I,
...
// Note: StateSynchronizer and ExecutionEngine<IRBlock> implementations removed
// to avoid circular dependency with vm-ir. These should be implemented
// in crates that depend on both vm-core and vm-ir.
...
pub trait PageTableManager {
    ...
    ) -> Result<crate::GuestPhysAddr, VmError>;

    /// 更新页表项
    fn update_entry(
        &mut self,
        vaddr: GuestAddr,
        paddr: crate::GuestPhysAddr,
        flags: PageFlags,
    ) -> Result<(), VmError>;
