...
impl ServiceRegistry {
    ...
    fn lock_services_read(&self) -> Result<std::sync::RwLockReadGuard<'_, HashMap<TypeId, Box<dyn ServiceDescriptor>>>, DIError> {
        self.services.read().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire services read lock: {}", e)
        ))
    }

    fn lock_services_write(&self) -> Result<std::sync::RwLockWriteGuard<'_, HashMap<TypeId, Box<dyn ServiceDescriptor>>>, DIError> {
        self.services.write().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire services write lock: {}", e)
        ))
    }

    fn lock_service_names_read(&self) -> Result<std::sync::RwLockReadGuard<'_, HashMap<String, TypeId>>, DIError> {
        self.service_names.read().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire service_names read lock: {}", e)
        ))
    }

    fn lock_service_names_write(&self) -> Result<std::sync::RwLockWriteGuard<'_, HashMap<String, TypeId>>, DIError> {
        self.service_names.write().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire service_names write lock: {}", e)
        ))
    }

    fn lock_service_types_read(&self) -> Result<std::sync::RwLockReadGuard<'_, HashMap<TypeId, String>>, DIError> {
        self.service_types.read().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire service_types read lock: {}", e)
        ))
    }

    fn lock_service_types_write(&self) -> Result<std::sync::RwLockWriteGuard<'_, HashMap<TypeId, String>>, DIError> {
        self.service_types.write().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire service_types write lock: {}", e)
        ))
    }

    fn lock_service_groups_read(&self) -> Result<std::sync::RwLockReadGuard<'_, HashMap<String, Vec<TypeId>>>, DIError> {
        self.service_groups.read().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire service_groups read lock: {}", e)
        ))
    }

    fn lock_service_groups_write(&self) -> Result<std::sync::RwLockWriteGuard<'_, HashMap<String, Vec<TypeId>>>, DIError> {
        self.service_groups.write().map_err(|e| DIError::InvalidServiceConfiguration(
            format!("Failed to acquire service_groups write lock: {}", e)
        ))
    }
}
...
impl ServiceRegistry {
    ...
    pub fn register_descriptor(&self, descriptor: Box<dyn ServiceDescriptor>) -> Result<(), DIError> {
        ...
        // 更新名称映射
        {
            let mut service_names = self.lock_service_names_write()?;
            let mut service_types = self.lock_service_types_write()?;
    ...
    pub fn register_named<T: 'static + Send + Sync>(
        ...
        // 检查名称是否已被使用
        {
            let service_names = self.lock_service_names_read()?;
    ...
    pub fn add_to_group<T: 'static + Send + Sync>(&self, group: &str) -> Result<(), DIError> {
        ...
        // 添加到分组
        {
            let mut service_groups = self.lock_service_groups_write()?;
    ...
    pub fn get_type_by_name(&self, name: &str) -> Option<TypeId> {
        match self.lock_service_names_read() {
            Ok(service_names) => service_names.get(name).cloned(),
            Err(_) => None,
        }
    }
    ...
    pub fn get_name_by_type(&self, type_id: TypeId) -> Option<String> {
        match self.lock_service_types_read() {
            Ok(service_types) => service_types.get(&type_id).cloned(),
            Err(_) => None,
        }
    }
    ...
    pub fn get_services_in_group(&self, group: &str) -> Vec<TypeId> {
        match self.lock_service_groups_read() {
            Ok(service_groups) => service_groups.get(group).cloned().unwrap_or_default(),
            Err(_) => Vec::new(),
        }
    }
    ...
    pub fn is_registered_name(&self, name: &str) -> bool {
        match self.lock_service_names_read() {
            Ok(service_names) => service_names.contains_key(name),
            Err(_) => false,
        }
    }
    ...
    pub fn service_names(&self) -> Vec<String> {
        match self.lock_service_names_read() {
            Ok(service_names) => service_names.keys().cloned().collect(),
            Err(_) => Vec::new(),
        }
    }
    ...
    pub fn service_groups(&self) -> Vec<String> {
        match self.lock_service_groups_read() {
            Ok(service_groups) => service_groups.keys().cloned().collect(),
            Err(_) => Vec::new(),
        }
    }
    ...
    pub fn unregister(&self, type_id: TypeId) -> Result<(), DIError> {
        ...
        // 获取服务名称
        let service_name = {
            let service_types = self.lock_service_types_read()?;
    ...
    /// 清空注册表
    pub fn clear(&self) {
        if let (Ok(mut services), Ok(mut service_names), Ok(mut service_types), Ok(mut service_groups)) = (
            self.lock_services_write(),
            self.lock_service_names_write(),
            self.lock_service_types_write(),
            self.lock_service_groups_write(),
        ) {
            services.clear();
            service_names.clear();
            service_types.clear();
    ...
    pub fn stats(&self) -> RegistryStats {
        match (self.lock_services_read(), self.lock_service_names_read(), self.lock_service_groups_read()) {
            (Ok(services), Ok(service_names), Ok(service_groups)) => {
            ...
            _ => RegistryStats::default(),
        }
    }
    
    /// 导出注册表配置
    pub fn export_config(&self) -> RegistryConfig {
        match (self.lock_services_read(), self.lock_service_types_read(), self.lock_service_groups_read()) {
            (Ok(services), Ok(service_types), Ok(service_groups)) => {
...
impl ServiceRegistryBuilder {
    ...
    pub fn with_singleton<T: 'static + Send + Sync>(self) -> Self {
        let _ = self.registry.register_singleton::<T>();
        self
    }

    /// 注册瞬态服务
    pub fn with_transient<T: 'static + Send + Sync>(self) -> Self {
        let _ = self.registry.register_transient::<T>();
        self
    }

    /// 注册作用域服务
    pub fn with_scoped<T: 'static + Send + Sync>(self) -> Self {
        let _ = self.registry.register_scoped::<T>();
        self
    }

    /// 注册服务实例
    pub fn with_instance<T: 'static + Send + Sync>(self, instance: T) -> Self {
        let _ = self.registry.register_instance(instance);
        self
    }
    ...
    where
        F: Fn(&dyn ServiceProvider) -> Result<T, DIError> + Send + Sync + 'static,
    {
        let _ = self.registry.register_factory(factory);
        self
    }

    /// 添加到分组
    pub fn with_group<T: 'static + Send + Sync>(self, group: &str) -> Self {
        let _ = self.registry.add_to_group::<T>(group);
        self
    }
...
mod tests {
    ...
    fn test_register_named() {
        ...
        let type_id = registry.get_type_by_name("test_string");
        assert_eq!(type_id, Some(TypeId::of::<String>()));

        let name = registry.get_name_by_type(TypeId::of::<String>());
        assert_eq!(name, Some("test_string".to_string()));
    }

    #[test]
    fn test_service_groups() {
        let registry = ServiceRegistry::new();
        let _ = registry.register_singleton::<String>();
        let _ = registry.add_to_group::<String>("test_group");
    ...
    fn test_unregister() {
        let registry = ServiceRegistry::new();
        let _ = registry.register_named::<String>("test_string", ServiceLifetime::Singleton);

        assert!(registry.is_registered::<String>());
        assert!(registry.is_registered_name("test_string"));

        let _ = registry.unregister_name("test_string");
