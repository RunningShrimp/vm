...
use vm_core::{GuestArch, GuestAddr, MemoryAccess};
...
impl CrossArchIntegrationTestFramework {
    ...
    fn test_basic_translation(&self, src_arch: GuestArch, dst_arch: GuestArch) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, byte) in test_code.iter().enumerate() {
            executor.mmu_mut().write(GuestAddr(code_base + i as u64), *byte as u64, 1)?;
        }
    ...
    fn test_complex_instruction_sequence(&self, src_arch: GuestArch, dst_arch: GuestArch) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 0..100 {
            executor.mmu_mut().write(GuestAddr(data_base + i as u64 * 8), i as u64, 8)?;
        }
    ...
    fn test_register_mapping(&self, src_arch: GuestArch, dst_arch: GuestArch) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 加载代码到内存
        for (i, byte) in test_code.iter().enumerate() {
            executor.mmu_mut().write(vm_core::GuestAddr(code_base + i as u64), *byte as u64, 1)?;
        }
    ...
    fn test_memory_access_patterns(&self, src_arch: GuestArch, dst_arch: GuestArch) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (base, size) in data_regions {
            for i in 0..size/8 {
                executor.mmu_mut().write(vm_core::GuestAddr(base + i as u64 * 8), (i * 7) as u64, 8)?;
            }
        }
    ...
    /// 测试SIMD指令
    fn test_simd(&self, src_arch: GuestArch, _dst_arch: GuestArch) -> Result<(), Box<dyn std::error::Error>> {
        // 创建执行器
        let mut executor = UnifiedExecutor::auto_create(src_arch, 1024 * 1024)?;
        ...
        let data_base = 0x10000;
        for i in 0..256 {
            executor.mmu_mut().write(vm_core::GuestAddr(data_base + i as u64), i as u64, 8)?;
        }
    ...
    fn test_multi_arch_interop(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, byte) in x86_code.iter().enumerate() {
            x86_executor.mmu_mut().write(vm_core::GuestAddr(0x1000 + i as u64), *byte as u64, 1)?;
        }
        
        for (i, byte) in arm_code.iter().enumerate() {
            arm_executor.mmu_mut().write(vm_core::GuestAddr(0x1000 + i as u64), *byte as u64, 1)?;
        }
        
        for (i, byte) in riscv_code.iter().enumerate() {
            riscv_executor.mmu_mut().write(vm_core::GuestAddr(0x1000 + i as u64), *byte as u64, 1)?;
        }
