...
impl CrossArchIntegrationTestFramework {
            ...
            GuestArch::X86_64 => {
                ...
                for i in 0..16 {
                    code.extend_from_slice(&[
                        0x48,
                        0x89,
                        0xC0 + i as u8, // mov r{i}, rax
                        0x48,
                        0x31,
                        0xC0 + i as u8, // xor r{i}, r{i}
                        0x48,
                        0x83,
                        0xC0 + i as u8,
                        0x01, // add r{i}, 1
        ...
        match arch {
            GuestArch::X86_64 => {
                let mut code = vec![
                    ...
                    0x48, 0x8B, 0x04, 0x25, 0x00, 0x00, 0x10,
                    0x00, // mov rax, [rip + 0x100000]
                    0x48, 0x8B, 0x1C, 0x25, 0x00, 0x00, 0x10,
                    0x08, // mov rbx, [rip + 0x100008]
                    0x48, 0x01, 0xD8, // add rax, rbx
                    0x48, 0x89, 0x04, 0x25, 0x00, 0x00, 0x10,
                    0x10, // mov [rip + 0x100010], rax
                    0xC3, // ret
                ];
    ...
    pub fn create_floating_point_test_code(&self, arch: GuestArch) -> Vec<u8> {
        match arch {
            GuestArch::X86_64 => {
                vec![
                    0xF2, 0x0F, 0x10, 0x05, 0x24, 0x00, 0x00,
                    0x00, // movsd xmm0, [rip + 0x24]
                    0xF2, 0x0F, 0x10, 0x0D, 0x2C, 0x00, 0x00,
                    0x00, // movsd xmm1, [rip + 0x2C]
                    0xF2, 0x0F, 0x58, 0xC1, // addsd xmm0, xmm0, xmm1
                    0xF2, 0x0F, 0x11, 0x05, 0x34, 0x00, 0x00,
                    0x00, // movsd [rip + 0x34], xmm0
                    0xC3, // ret
                ]
            }
            GuestArch::Arm64 => {
    ...
    pub fn create_simd_test_code(&self, arch: GuestArch) -> Vec<u8> {
        match arch {
            GuestArch::X86_64 => {
                let mut code = vec![
                    0xC5, 0xF9, 0x6F, 0x05, 0x24, 0x00, 0x00,
                    0x00, // vmovdqu ymm0, [rip + 0x24]
                    0xC5, 0xF9, 0x6F, 0x0D, 0x44, 0x00, 0x00,
                    0x00, // vmovdqu ymm1, [rip + 0x44]
                    0xC5, 0xFD, 0xD4, 0xC1, // vpaddq ymm0, ymm0, ymm1
                    0xC5, 0xFD, 0x7F, 0x05, 0x64, 0x00, 0x00,
                    0x00, // vmovdqu [rip + 0x64], ymm0
                    0xC3, // ret
                ];
                ...
                for i in 0..5 {
                    code.extend_from_slice(&[
                        ...
                        0xF9,
                        0x6F,
                        0x05 + ((i * 0x20) & 0xFF) as u8,
                        0x84 + ((i * 0x20) >> 8) as u8,
                        0x00,
                        0x00, // vmovdqu ymm0, [rip + offset]
                        0xC5,
                        0xF9,
                        0x6F,
                        0x0D + ((i * 0x20) & 0xFF) as u8,
                        0xA4 + ((i * 0x20) >> 8) as u8,
                        0x00,
                        0x00, // vmovdqu ymm1, [rip + offset]
                        0xC5,
                        0xFD,
                        0xD4,
                        0xC1, // vpaddq ymm0, ymm0, ymm1
                        0xC5,
                        0xFD,
                        0x7F,
                        0x05 + ((i * 0x20) & 0xFF) as u8,
            ...
            GuestArch::Arm64 => {
                ...
                for i in 0..5 {
                    code.extend_from_slice(&[
                        0x40 + ((i >> 2) & 0x1) as u8,
                        0x04,
                        0x40 + ((i * 0x20) & 0xFF) as u8,
                        0x4C, // ld1 v0.16b, [x2, #offset]
                        0x41 + ((i >> 2) & 0x1) as u8,
                        0x04,
                        0x40 + ((i * 0x20) & 0xFF) as u8,
                        0x4C, // ld1 v1.16b, [x2, #offset]
                        0x20 + (i & 0x3) as u8,
                        0x04,
                        ((i & 0x3) as u8),
                        0x4E, // add v0.16b, v0.16b, v1.16b
                        0x40 + ((i >> 2) & 0x1) as u8,
                        0x04,
                        (((i * 0x20) & 0xFF) as u8),
                        0x4C, // st1 v0.16b, [x2, #offset]
                    ]);
                }
            ...
            GuestArch::Riscv64 => {
                ...
                for i in 0..5 {
                    code.extend_from_slice(&[
                        ...
                        0x07,
                        0xA0 + ((i >> 2) & 0x1) as u8,
                        0x02, // vsetvli a0, x0, e8, m1
                        0x07,
                        0x07,
                        0xA0 + ((i * 0x20) & 0xFF) as u8,
                        0x42, // vle8.v v8, (x2)
                        0x07,
                        0x07,
                        0xA1 + ((i * 0x20) & 0xFF) as u8,
                        0x42, // vle8.v v9, (x2)
                        0x57,
                        0x45 + (i & 0x3) as u8,
                        0x08,
                        0x02, // vadd.vv v8, v8, v9
                        0x27,
                        0x07,
                        0xA0 + ((i * 0x20) & 0xFF) as u8,
                        0x42, // vse8.v v8, (x2)
