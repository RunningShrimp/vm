...
impl MonitoringService {
    ...
    pub async fn get_system_metrics(&self) -> Result<crate::ipc::SystemMetrics, String> {
        use crate::ipc::SystemMetrics;
    ...
    pub async fn start_collection(&self, vm_id: String) -> Result<(), String> {
        ...
        // Record start time
        {
            let mut start_times = self
                .start_times
                .lock()
                .map_err(|e| format!("Failed to acquire start_times lock: {}", e))?;
        ...
        let task = tokio::spawn(async move {
            ...
            loop {
                ...
                let uptime = {
                    let start_times = match start_times.lock() {
                        Ok(guard) => guard,
                        Err(e) => {
                            debug!("Failed to acquire start_times lock for uptime: {}", e);
                            return; // Exit the task on poisoned mutex
                        }
                    };
                    start_times
                        .get(&vm_id_clone)
        ...
        let mut tasks = self
            .collection_tasks
            .lock()
            .map_err(|e| format!("Failed to acquire collection_tasks lock: {}", e))?;
    ...
    fn collect_system_metrics(
        ...
        let cpu_usage =
            ((*last_cpu_time % 100) as f32 * 0.3 + rand::random::<f32>() * 70.0).clamp(5.0, 95.0);
