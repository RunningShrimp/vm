...
pub struct GpuCommandQueue {
    queue: Mutex<VecDeque<GpuCommand>>,
    max_size: usize,
    state: Mutex<GpuQueueState>,
    stats: Mutex<GpuQueueStats>,
    cond: Condvar,
}
...
impl GpuCommandQueue {
    ...
    pub fn new(max_size: usize) -> Self {
        Self {
            ...
            max_size,
            state: Mutex::new(GpuQueueState::Idle),
            stats: Mutex::new(GpuQueueStats {
                total_submitted: 0,
                total_completed: 0,
                avg_wait_time_us: 0,
                max_queue_depth: 0,
...
pub struct GpuDeviceSimulator {
    ...
    memory_blocks: Mutex<HashMap<String, GpuMemoryBlock>>,
    state: Mutex<GpuQueueState>,
...
impl GpuDeviceSimulator {
    ...
    pub fn new(device_info: GpuDeviceInfo) -> Self {
        ...
        Self {
            ...
            memory_blocks: Mutex::new(HashMap::new()),
            state: Mutex::new(GpuQueueState::Idle),
            memory_allocator,
        }
    }
