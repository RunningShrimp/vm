...
use vm_engine_jit::dynamic_optimization::{DynamicOptimizationManager, DynamicOptimizationConfig};
use vm_engine_jit::advanced_benchmark::{AdvancedPerformanceBenchmarker, BenchmarkConfig, BenchmarkTestCaseGenerator};
use vm_engine_jit::performance_benchmark::PerformanceBenchmarker;
use vm_engine_jit::adaptive_threshold::{AdaptiveThresholdManager, AdaptiveThresholdConfig};
...
fn test_multithreaded_compilation() {
    ...
    // 异步编译所有IR块
    let task_ids: Vec<u64> = ir_blocks.into_iter()
        .enumerate()
        .map(|(i, block)| {
...
fn test_multithreaded_compilation_with_callback() {
    ...
    compiler.compile_async_with_callback(ir_block, TaskPriority::High, move |result| {
...
fn test_dynamic_optimization_manager() {
    ...
    let manager = DynamicOptimizationManager::new(
        jit_engine.clone(),
        threshold_manager,
        dynamic_config,
    );
