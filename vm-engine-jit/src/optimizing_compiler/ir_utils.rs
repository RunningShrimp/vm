...
impl IrAnalyzer {
    ...
    pub fn collect_read_regs(op: &IROp) -> Vec<RegId> {
        match op {
            ...
            IROp::Select { cond, true_val, false_val, .. } => vec![*cond, *true_val, *false_val],
            ...
            IROp::AtomicCmpXchg { base, expected, new, .. } => vec![*base, *expected, *new],
            IROp::AtomicCmpXchgOrder { base, expected, new, .. } => vec![*base, *expected, *new],
            IROp::AtomicLoadReserve { base, .. } => vec![*base],
            IROp::AtomicStoreCond { src, base, dst_flag, .. } => vec![*src, *base, *dst_flag],
            IROp::AtomicCmpXchgFlag { base, expected, new, .. } => vec![*base, *expected, *new],
            ...
            IROp::Vec128Add { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Sub { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Mul { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Div { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Rem { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128And { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Or { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Xor { src1_lo, src1_hi, src2_lo, src2_hi, .. } => vec![*src1_lo, *src1_hi, *src2_lo, *src2_hi],
            IROp::Vec128Not { src_lo, src_hi, .. } => vec![*src_lo, *src_hi],
            IROp::Vec128Shl { src_lo, src_hi, sh_lo, sh_hi, .. } => vec![*src_lo, *src_hi, *sh_lo, *sh_hi],
            IROp::Vec128Srl { src_lo, src_hi, sh_lo, sh_hi, .. } => vec![*src_lo, *src_hi, *sh_lo, *sh_hi],
            IROp::Vec128Sra { src_lo, src_hi, sh_lo, sh_hi, .. } => vec![*src_lo, *src_hi, *sh_lo, *sh_hi],
            IROp::Vec128ShlImm { src_lo, src_hi, .. } => vec![*src_lo, *src_hi],
            IROp::Vec128SrlImm { src_lo, src_hi, .. } => vec![*src_lo, *src_hi],
            IROp::Vec128SraImm { src_lo, src_hi, .. } => vec![*src_lo, *src_hi],
            IROp::Vec256Add { src10, src11, src12, src13, src20, src21, src22, src23, .. } =>
                vec![*src10, *src11, *src12, *src13, *src20, *src21, *src22, *src23],
            IROp::Vec256Sub { src10, src11, src12, src13, src20, src21, src22, src23, .. } =>
                vec![*src10, *src11, *src12, *src13, *src20, *src21, *src22, *src23],
            IROp::Vec256Mul { src10, src11, src12, src13, src20, src21, src22, src23, .. } =>
                vec![*src10, *src11, *src12, *src13, *src20, *src21, *src22, *src23],
            ...
            IROp::Fmadd { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::Fmsub { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::Fnmadd { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::Fnmsub { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::FmaddS { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::FmsubS { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::FnmaddS { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
            IROp::FnmsubS { src1, src2, src3, .. } => vec![*src1, *src2, *src3],
    ...
    pub fn collect_written_regs(op: &IROp) -> Vec<RegId> {
        match op {
            ...
            IROp::AtomicCmpXchgFlag { dst_old, dst_flag, .. } => vec![*dst_old, *dst_flag],
            IROp::AtomicRmwFlag { dst_old, dst_flag, .. } => vec![*dst_old, *dst_flag],
            ...
            IROp::Vec128Sub { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Mul { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Div { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Rem { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128And { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Or { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Xor { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Not { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Shl { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Srl { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128Sra { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128ShlImm { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128SrlImm { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec128SraImm { dst_lo, dst_hi, .. } => vec![*dst_lo, *dst_hi],
            IROp::Vec256Add { dst0, dst1, dst2, dst3, .. } => vec![*dst0, *dst1, *dst2, *dst3],
            IROp::Vec256Sub { dst0, dst1, dst2, dst3, .. } => vec![*dst0, *dst1, *dst2, *dst3],
            IROp::Vec256Mul { dst0, dst1, dst2, dst3, .. } => vec![*dst0, *dst1, *dst2, *dst3],
            ...
            IROp::Cpuid { dst_eax, dst_ebx, dst_ecx, dst_edx, .. } => vec![*dst_eax, *dst_ebx, *dst_ecx, *dst_edx],
    ...
    pub fn is_pure(op: &IROp) -> bool {
        match op {
            IROp::Add { .. } | IROp::Sub { .. } | IROp::Mul { .. } | IROp::Div { .. } | IROp::Rem { .. } => true,
            ...
            IROp::CmpEq { .. } | IROp::CmpNe { .. } | IROp::CmpLt { .. } | IROp::CmpLtU { .. } => true,
            ...
            IROp::Vec128Add { .. } | IROp::Vec128Sub { .. } | IROp::Vec128Mul { .. } => true,
            IROp::Vec128Div { .. } | IROp::Vec128Rem { .. } => true,
            IROp::Vec128And { .. } | IROp::Vec128Or { .. } | IROp::Vec128Xor { .. } => true,
            IROp::Vec128Not { .. } | IROp::Vec128Shl { .. } | IROp::Vec128Srl { .. } | IROp::Vec128Sra { .. } => true,
            IROp::Vec128ShlImm { .. } | IROp::Vec128SrlImm { .. } | IROp::Vec128SraImm { .. } => true,
            ...
            IROp::FaddS { .. } | IROp::FsubS { .. } | IROp::FmulS { .. } | IROp::FdivS { .. } => true,
            IROp::FsqrtS { .. } | IROp::FminS { .. } | IROp::FmaxS { .. } => true,
            IROp::Fmadd { .. } | IROp::Fmsub { .. } | IROp::Fnmadd { .. } | IROp::Fnmsub { .. } => true,
            IROp::FmaddS { .. } | IROp::FmsubS { .. } | IROp::FnmaddS { .. } | IROp::FnmsubS { .. } => true,
            IROp::Feq { .. } | IROp::Flt { .. } | IROp::Fle { .. } => true,
            IROp::FeqS { .. } | IROp::FltS { .. } | IROp::FleS { .. } => true,
            IROp::Fcvtws { .. } | IROp::Fcvtwus { .. } | IROp::Fcvtls { .. } | IROp::Fcvtlus { .. } => true,
            IROp::Fcvtsw { .. } | IROp::Fcvtswu { .. } | IROp::Fcvtsl { .. } | IROp::Fcvtslu { .. } => true,
            IROp::Fcvtwd { .. } | IROp::Fcvtwud { .. } | IROp::Fcvtld { .. } | IROp::Fcvtlud { .. } => true,
            IROp::Fcvtdw { .. } | IROp::Fcvtdwu { .. } | IROp::Fcvtdl { .. } | IROp::Fcvtdlu { .. } => true,
            ...
            IROp::FmvXW { .. } | IROp::FmvWX { .. } | IROp::FmvXD { .. } | IROp::FmvDX { .. } => true,
    ...
    pub fn is_arithmetic(op: &IROp) -> bool {
        matches!(op, IROp::Add { .. } | IROp::Sub { .. } | IROp::Mul { .. } | IROp::Div { .. })
    }
    ...
    pub fn get_complexity_score(op: &IROp) -> u32 {
        match op {
            ...
            IROp::Vec128Add { .. } | IROp::Vec128Sub { .. } | IROp::Vec128Mul { .. } => 6,
            IROp::Vec128Div { .. } | IROp::Vec128Rem { .. } => 8,
            IROp::Vec128And { .. } | IROp::Vec128Or { .. } | IROp::Vec128Xor { .. } => 5,
            IROp::Vec128Not { .. } | IROp::Vec128Shl { .. } | IROp::Vec128Srl { .. } | IROp::Vec128Sra { .. } => 5,
            IROp::Vec128ShlImm { .. } | IROp::Vec128SrlImm { .. } | IROp::Vec128SraImm { .. } => 4,
            ...
            IROp::Fmadd { .. } | IROp::Fmsub { .. } | IROp::Fnmadd { .. } | IROp::Fnmsub { .. } => 6,
            IROp::FmaddS { .. } | IROp::FmsubS { .. } | IROp::FnmaddS { .. } | IROp::FnmsubS { .. } => 6,
            IROp::Feq { .. } | IROp::Flt { .. } | IROp::Fle { .. } => 3,
            IROp::FeqS { .. } | IROp::FltS { .. } | IROp::FleS { .. } => 3,
            IROp::Fcvtws { .. } | IROp::Fcvtwus { .. } | IROp::Fcvtls { .. } | IROp::Fcvtlus { .. } => 5,
            IROp::Fcvtsw { .. } | IROp::Fcvtswu { .. } | IROp::Fcvtsl { .. } | IROp::Fcvtslu { .. } => 5,
            IROp::Fcvtwd { .. } | IROp::Fcvtwud { .. } | IROp::Fcvtld { .. } | IROp::Fcvtlud { .. } => 5,
            IROp::Fcvtdw { .. } | IROp::Fcvtdwu { .. } | IROp::Fcvtdl { .. } | IROp::Fcvtdlu { .. } => 5,
    ...
    pub fn get_latency_cycles(op: &IROp) -> u32 {
        match op {
            ...
            IROp::VecAddSat { .. } | IROp::VecSubSat { .. } | IROp::VecMulSat { .. } => 5,
            IROp::Vec128Add { .. } | IROp::Vec128Sub { .. } | IROp::Vec128Mul { .. } => 6,
            IROp::Vec128Div { .. } | IROp::Vec128Rem { .. } => 12,
            IROp::Vec128And { .. } | IROp::Vec128Or { .. } | IROp::Vec128Xor { .. } => 4,
            IROp::Vec128Not { .. } | IROp::Vec128Shl { .. } | IROp::Vec128Srl { .. } | IROp::Vec128Sra { .. } => 4,
            IROp::Vec128ShlImm { .. } | IROp::Vec128SrlImm { .. } | IROp::Vec128SraImm { .. } => 3,
            IROp::Vec256Add { .. } | IROp::Vec256Sub { .. } | IROp::Vec256Mul { .. } => 8,
            IROp::Fadd { .. } | IROp::Fsub { .. } | IROp::Fmul { .. } | IROp::Fdiv { .. } => 4,
            IROp::Fsqrt { .. } | IROp::Fmin { .. } | IROp::Fmax { .. } => 6,
            IROp::FaddS { .. } | IROp::FsubS { .. } | IROp::FmulS { .. } | IROp::FdivS { .. } => 4,
            IROp::FsqrtS { .. } | IROp::FminS { .. } | IROp::FmaxS { .. } => 6,
            IROp::Fmadd { .. } | IROp::Fmsub { .. } | IROp::Fnmadd { .. } | IROp::Fnmsub { .. } => 7,
            IROp::FmaddS { .. } | IROp::FmsubS { .. } | IROp::FnmaddS { .. } | IROp::FnmsubS { .. } => 7,
            IROp::Feq { .. } | IROp::Flt { .. } | IROp::Fle { .. } => 3,
            IROp::FeqS { .. } | IROp::FltS { .. } | IROp::FleS { .. } => 3,
            IROp::Fcvtws { .. } | IROp::Fcvtwus { .. } | IROp::Fcvtls { .. } | IROp::Fcvtlus { .. } => 8,
            IROp::Fcvtsw { .. } | IROp::Fcvtswu { .. } | IROp::Fcvtsl { .. } | IROp::Fcvtslu { .. } => 8,
            IROp::Fcvtwd { .. } | IROp::Fcvtwud { .. } | IROp::Fcvtld { .. } | IROp::Fcvtlud { .. } => 8,
            IROp::Fcvtdw { .. } | IROp::Fcvtdwu { .. } | IROp::Fcvtdl { .. } | IROp::Fcvtdlu { .. } => 8,
