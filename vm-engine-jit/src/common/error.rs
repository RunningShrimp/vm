...
use vm_foundation::{
    ErrorContext, ErrorContextExt, JitError, VmError, VmResult, error_utils as utils,
};
        ...
        for error in &self.error_history {
            ...
            *count += 1;
            ...
            let error_type = match error {
                ...
                VmError::Io { .. } => "I/O",
                VmError::Generic { .. } | VmError::GenericMsg(_) => "Generic",
            };
...
impl JITErrorRecoveryManager {
    ...
    pub fn handle_error(&mut self, error: &VmError) -> JITRecoveryAction {
        match self.strategy {
            ...
            JITErrorRecoveryStrategy::Retry {
                max_attempts,
                delay_ms,
            } => {
...
impl JITErrorBuilder {
    ...
    pub fn config(message: impl Into<String>) -> VmError {
        VmError::Configuration {
            source: vm_foundation::ConfigError::InvalidValue("config".to_string(), message.into()),
    ...
    pub fn memory(message: impl Into<String>) -> VmError {
        VmError::Memory {
            source: vm_foundation::MemoryError::AllocationFailed(message.into()),
    ...
    pub fn hardware_acceleration(message: impl Into<String>) -> VmError {
        VmError::Device {
            source: vm_foundation::DeviceError::InitializationFailed(message.into()),
    ...
    pub fn performance_analysis(message: impl Into<String>) -> VmError {
        VmError::Core {
            source: vm_foundation::CoreError::UnsupportedOperation(message.into()),
    ...
    pub fn resource_exhausted(resource: impl Into<String>) -> VmError {
        VmError::Core {
            source: vm_foundation::CoreError::ResourceNotAvailable(resource.into()),
    ...
    pub fn timeout(operation: impl Into<String>) -> VmError {
        VmError::Core {
            source: vm_foundation::CoreError::Timeout(operation.into()),
    ...
    pub fn concurrency(message: impl Into<String>) -> VmError {
        VmError::Core {
            source: vm_foundation::CoreError::ResourceInUse(message.into()),
...
mod tests {
    ...
    fn test_jit_error_recovery() {
        let mut recovery = JITErrorRecoveryManager::new(JITErrorRecoveryStrategy::Retry {
            max_attempts: 3,
            delay_ms: 100,
        });
