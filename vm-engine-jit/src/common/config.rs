...
impl Config for BaseConfig {
    ...
    fn summary(&self) -> String {
        format!(
            ...
            self.log_level,
            self.worker_threads,
            self.max_memory_bytes.map(|bytes| bytes / (1024 * 1024)).unwrap_or(0),
    ...
    fn merge(&mut self, other: &Self) {
        ...
        if other.worker_threads != num_cpus::get() {
...
impl ConfigManager {
    ...
    /// 添加配置
    pub fn add_config<C: Config + Serialize + 'static>(&mut self, name: &str, config: C) -> Result<(), serde_json::Error> {
    ...
    /// 获取配置
    pub fn get_config<C: serde::de::DeserializeOwned>(&self, name: &str) -> Result<Option<C>, serde_json::Error> {
    ...
    /// 获取可变配置
    pub fn get_config_mut<C: serde::de::DeserializeOwned + serde::Serialize>(&mut self, name: &str) -> Result<Option<C>, serde_json::Error> {
        match self.configs.get(name) {
            Some(json) => {
                let config: C = serde_json::from_str(json)?;
                Ok(Some(config))
    ...
    pub fn validate_all(&self) -> Result<(), String> {
        ...
        for (name, json) in &self.configs {
            ...
            if let Ok(_) = serde_json::from_str::<BaseConfig>(json) {
                // 基本JSON格式正确
            } else {
...
mod tests {
    ...
    fn test_hardware_acceleration_config() {
        ...
        assert_eq!(config.performance_monitoring_interval, Duration::from_secs(5));
