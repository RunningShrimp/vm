...
use vm_core::{GuestAddr, VmError, ExecResult, ExecStats, ExecStatus, MMU};
use vm_ir::IRBlock;
...
// use crate::advanced_cache::{AdvancedCodeCache, AdvancedCacheConfig};
// use crate::advanced_optimizer::{AdvancedOptimizer, AdvancedOptimizerConfig};
// use crate::performance_analyzer::{JITPerformanceAnalyzer, PerformanceAnalyzerConfig, PerformanceReport};
// use crate::hw_acceleration::{HardwareAccelerationManager, HardwareAccelerationConfig};
...
impl JITEngine {
    ...
    pub fn new(config: JITConfig) -> Self {
        ...
        Self {
            ...
            advanced_optimizer: Some(Box::new(crate::optimizer::DefaultIROptimizer::new(config.clone()))),
            code_cache,
    ...
    /// 初始化并行编译
    fn init_parallel_compilation(config: &JITConfig) -> (Option<Sender<CompilationTask>>, Option<Receiver<CompilationTaskResult>>, Vec<thread::JoinHandle<()>>) {
        // 使用优先级队列来存储编译任务
    ...
    fn compilation_worker(
        thread_id: usize,
        task_queue: Arc<Mutex<std::collections::BinaryHeap<(std::cmp::Reverse<u32>, CompilationTask)>>>,
    ...
    fn compile_block_internal(
        ...
        // 1. 优化IR
        let mut optimized_block = if config.enable_optimization {
        ...
        if config.enable_simd {
            if let Err(e) = simd_optimizer.optimize(&mut optimized_block) {
        ...
        // 4. 指令调度
        let scheduled_block = if matches!(config.instruction_scheduling_strategy, 
    ...
    fn compile_parallel(&mut self, block: &IRBlock) -> Result<JITCompilationResult, VmError> {
        ...
        // 发送编译任务
        if let Err(_) = sender.send(task_clone) {
            // 发送失败，回退到顺序编译
            {
                let mut pending = self.pending_tasks.lock().unwrap();
                    ...
                    Ok(mut result) => {
                        if result.task_id == task_id {
                            ...
                            if let Ok(mut stats) = self.compilation_stats.lock() {
                                stats.compilation_time_ns += result.compilation_time.as_nanos() as u64;
    ...
    fn compile_sequential(&mut self, block: &IRBlock) -> Result<JITCompilationResult, VmError> {
        ...
        // 1. 优化IR
        let mut optimized_block = if self.config.enable_optimization {
        ...
        if self.config.enable_simd {
            ...
            if let Err(e) = self.simd_optimizer.optimize(&mut optimized_block) {
                eprintln!("SIMD optimization failed: {}", e);
            }
        ...
        if self.config.enable_optimization {
            ...
            if let Some(ref mut optimizer) = self.advanced_optimizer {
                if let Err(e) = optimizer.optimize(&mut optimized_block) {
        ...
        // 3. 指令调度
        let scheduled_block = if matches!(self.config.instruction_scheduling_strategy, 
                                        InstructionSchedulingStrategy::NoScheduling) {
            allocated_block
        } else {
    ...
    pub fn is_hotspot(&self, pc: GuestAddr) -> bool {
        if let Ok(counter) = self.hotspot_counter.lock() {
            counter.get(&pc).map_or(false, |&count| count >= self.config.hotspot_threshold)
        } else {
            false
        }
    }
    ...
    fn get_ir_block(&self, pc: GuestAddr) -> Result<IRBlock, VmError> {
        ...
        let ops = vec![
            ...
            IROp::Add { dst: 3u32, src1: 1u32, src2: 2u32 },
            IROp::MovImm { dst: 4u32, imm: pc.0 },
        ];
