...
//! - 编译代码池管理
//! - [`ExecutableBlock`][ExecutableBlock]: 强类型可执行代码块模型
//!
//! ## 可执行块模型
//!
//! 为确保类型安全和内存安全，我们使用强类型的 [`ExecutableBlock`] 而不是裸的 `Vec<u8>`
//! 来表示编译后的机器码。
//!
//! ## 热点追踪
//!
...
use vm_core::{ExecResult, ExecStats, ExecStatus, ExecutionEngine, GuestAddr, MMU};
...
// cranelift_native is used via cranelift_native::builder() call
use parking_lot::Mutex;
use std::collections::HashMap;
use std::sync::Arc;
// pub use vendor_optimizations::{VendorOptimizationStrategy, VendorOptimizer};

// Minimal module surface for incremental compilation and focused fixes
// Keep only modules that currently have implementation files. Other modules
// will be re-enabled once their source files are restored or implemented.

pub mod adaptive_compile_queue;
...
// Lightweight stubs to allow incremental compilation
...
// ML引导编译器和PGO优化器模块（从ml-guided-compiler和pgo-optimizer合并）
pub mod ml_guided_compiler;
pub use ml_guided_compiler::{
    ABTestFramework, ABTestMetrics, ABTestResults, BlockFeatures,
    CompilationDecision as MLCompilationDecision, MLCompilerStats, MLGuidedCompiler,
    SimpleLinearModel,
};

pub mod pgo_optimizer;
pub use pgo_optimizer::{
    AotOptimizationDriver, AotOptimizationHint, BlockProfile as PgoBlockProfile, CallProfile,
    PgoIterationResult, PgoManager, PgoOptimizationStats, ProfileCollector as PgoProfileCollector,
    ProfileStats as PgoProfileStats,
};

#[cfg(not(feature = "experimental"))]
mod ml_stubs {
    #[derive(Debug, Clone, Copy)]
    #[allow(dead_code)] // Variants used when experimental feature is enabled
...
#[cfg(not(feature = "experimental"))]
#[allow(unused_imports)] // Import is conditionally used via cfg attributes
use ml_stubs as ml_guided_jit;

pub mod loop_opt;

// Other modules temporarily disabled to allow incremental work on JIT
// (block_chaining, cranelift_backend, allocator, etc.)
...
/// 已编译代码块
///
/// 表示从IR编译得到的机器码，包含元数据和安全保证。
#[derive(Clone, Debug)]
pub struct CompiledBlock {
    /// 代码块的起始地址
    pub start_pc: vm_core::GuestAddr,
    /// 代码块的大小（字节数）
    pub size: usize,
    /// 代码块的哈希值（用于验证）
    pub hash: u64,
...
impl ExecutableMemory {
    /// 创建新的可执行内存区域
    pub fn new(size: usize) -> Result<Self, vm_core::VmError> {
        // 使用系统分配器分配可执行内存
        let layout = std::alloc::Layout::from_size_align(size, std::mem::align_of::<u8>())
            .map_err(|_| {
                vm_core::VmError::Memory(vm_core::MemoryError::AllocationFailed {
                    message: "Invalid allocation".to_string(),
                    size: None,
                })
            })?;
        ...
        if ptr.is_null() {
            return Err(vm_core::VmError::Memory(
                vm_core::MemoryError::AllocationFailed {
        ...
        unsafe {
            ...
            let aligned_size =
                ((ptr as usize + size + page_size - 1) & !(page_size - 1)) - (aligned_ptr as usize);
            mprotect(
                aligned_ptr,
                aligned_size,
    ...
    /// 分配内存区域用于存储编译后的代码
    pub fn allocate(&mut self, size: usize) -> Result<ExecutableBlockRef, vm_core::VmError> {
        if self.freed {
            return Err(vm_core::VmError::Memory(
                vm_core::MemoryError::AccessViolation {
                    addr: vm_core::GuestAddr(0),
                    msg: "Cannot allocate from freed memory".to_string(),
                    access_type: None,
                },
            ));
        }
        ...
        if self.used + size > self.size {
            return Err(vm_core::VmError::Memory(
                vm_core::MemoryError::AllocationFailed {
                    message: "Insufficient memory space".to_string(),
                    size: Some(size),
                },
            ));
        }
        ...
        Ok(ExecutableBlockRef {
            memory: self as *mut ExecutableMemory,
            offset,
            size,
        })
    }
...
impl ExecutableBlockRef {
    /// 获取代码块的入口点指针
    pub fn entry_ptr(&self) -> *const u8 {
        unsafe {
            let memory = &*self.memory;
            memory.base_ptr.add(self.offset)
        }
    }
...
unsafe impl Send for ExecutableBlockRef {}
unsafe impl Sync for ExecutableBlockRef {}

/// 强类型可执行代码块
///
/// 替代裸的 `Vec<u8>`，提供类型安全和内存安全的机器码表示。
/// 包含编译后的机器码、入口点和元数据信息。
#[derive(Clone, Debug)]
...
impl ExecutableBlock {
    /// 创建新的可执行块
    pub fn new(code: Vec<u8>, entry_offset: usize) -> Self {
...
extern "C" fn jit_read(ctx: *mut JitContext, vaddr: u64, size: u8) -> u64 {
    unsafe {
        let pa = match (*ctx)
            .mmu
            .translate(vm_core::GuestAddr(vaddr), vm_core::AccessType::Read)
        {
            Ok(p) => p,
            Err(_) => return 0,
        };
        (*ctx).mmu.read(vm_core::GuestAddr(pa.0), size).unwrap_or(0)
    }
}
...
extern "C" fn jit_write(ctx: *mut JitContext, vaddr: u64, val: u64, size: u8) {
    unsafe {
        if let Ok(pa) = (*ctx)
            .mmu
            .translate(vm_core::GuestAddr(vaddr), vm_core::AccessType::Write)
        {
            let _ = (*ctx).mmu.write(vm_core::GuestAddr(pa.0), val, size);
        }
    }
}
...
extern "C" fn jit_lr(ctx: *mut JitContext, vaddr: u64, size: u8) -> u64 {
    unsafe {
        (*ctx)
            .mmu
            .load_reserved(vm_core::GuestAddr(vaddr), size)
            .unwrap_or(0)
    }
}
...
extern "C" fn jit_sc(ctx: *mut JitContext, vaddr: u64, val: u64, size: u8) -> u64 {
    unsafe {
        match (*ctx)
            .mmu
            .store_conditional(vm_core::GuestAddr(vaddr), val, size)
        {
            Ok(true) => 1,
            Ok(false) => 0,
            Err(_) => 0,
...
extern "C" fn jit_cas(ctx: *mut JitContext, vaddr: u64, expected: u64, new: u64, size: u8) -> u64 {
    unsafe {
        ...
        let old = (*ctx).mmu.read(pa_r.into(), size).unwrap_or(0);
        if old == expected
            && let Ok(pa_w) = (*ctx)
                .mmu
                .translate(vm_core::GuestAddr(vaddr), vm_core::AccessType::Write)
        {
            let _ = (*ctx).mmu.write(pa_w.into(), new, size);
        }
        std::sync::atomic::fence(std::sync::atomic::Ordering::SeqCst);
        old
    }
...
/// 编译代码引用
///
/// 强类型替代原有的CodePtr，提供更好的类型安全。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CodeRef {
    /// 可执行代码块引用
    block_ref: Option<ExecutableBlockRef>,
}
...
impl CodeRef {
    ...
    /// 从可执行块引用创建代码引用
    pub fn from_block_ref(block_ref: ExecutableBlockRef) -> Self {
        Self {
    ...
    pub fn is_null(&self) -> bool {
        self.block_ref.is_none()
    }

    /// 从裸指针创建代码引用（向后兼容）
    pub fn from_ptr(ptr: *const u8) -> Self {
        // 临时实现：创建虚拟的ExecutableBlockRef
        // TODO: 集成真正的ExecutableMemory系统
        if ptr.is_null() {
            Self::null()
        } else {
            // 创建一个虚拟的block_ref（这不是最优的实现）
            let dummy_memory = Box::into_raw(Box::new(ExecutableMemory {
            ...
            let block_ref = ExecutableBlockRef {
                memory: dummy_memory,
                offset: 0,
                size: 0, // 未知大小
            };
    ...
    pub fn entry_ptr(&self) -> *const u8 {
        match self.block_ref {
            Some(ref block_ref) => block_ref.entry_ptr(),
            None => std::ptr::null(),
        }
    }
    ...
    /// 获取代码块大小
    pub fn size(&self) -> usize {
        self.block_ref.as_ref().map_or(0, |r| r.size())
    }
}
...
impl ShardedCache {
    ...
    /// 获取总条目数
    #[allow(dead_code)] // Reserved for future cache statistics
    fn len(&self) -> usize {
...
pub struct Jit {
    ...
    /// 循环优化器
    loop_optimizer: crate::loop_opt::LoopOptimizer,
    /// 事件总线（可选，用于发布领域事件）
    ...
    /// 待编译队列（用于增量编译）- 使用自适应队列
    pending_compile_queue: adaptive_compile_queue::AdaptiveCompileQueue,
    /// 预编译队列（用于代码预取）- 使用自适应队列
    prefetch_compile_queue: adaptive_compile_queue::AdaptiveCompileQueue,
    /// 编译时间预算（纳秒）
...
// NOTE: The `JITModule` from cranelift contains non-Sync interior types. For
// incremental compilation purposes we assert Jit is Send+Sync. This should be
// reviewed: a safer approach is to wrap non-Sync fields with `Mutex` or refactor
// to avoid sharing non-thread-safe data across threads.
...
impl Jit {
    ...
    fn create_jit_without_ml() -> Self {
        ...
        #[cfg(feature = "experimental")]
        {
            // SIMD functions would be registered here if available
        ...
        Self {
            ...
            loop_optimizer: crate::loop_opt::LoopOptimizer::default(),
            event_bus: None,
            vm_id: None,
            ...
            pending_compile_queue: adaptive_compile_queue::AdaptiveCompileQueue::default(),
            prefetch_compile_queue: adaptive_compile_queue::AdaptiveCompileQueue::default(),
    ...
    pub fn save_profile_data<P: AsRef<std::path::Path>>(&self, path: P) -> Result<(), String> {
        if let Some(ref collector) = self.profile_collector {
            collector.serialize_to_file(path).map_err(|e| e.to_string())
        } else {
    ...
    /// 获取ML编译决策
    #[cfg(feature = "experimental")]
    pub fn get_ml_decision(&self, block: &IRBlock) -> Option<ml_guided_jit::CompilationDecision> {
    ...
    pub fn record_ml_sample(
        &self,
        block: &IRBlock,
        decision: ml_guided_jit::CompilationDecision,
        performance: f64,
    ) {
    ...
    pub fn record_execution(&mut self, pc: GuestAddr) -> bool {
        ...
        if should_compile {
            // 计算编译优先级（基于关键路径和调用频率）
            let priority_value = self.compute_compile_priority(pc);
            // 转换为 CompilePriority
            let priority = match priority_value {
                0..=1 => adaptive_compile_queue::CompilePriority::Low,
                2..=5 => adaptive_compile_queue::CompilePriority::Medium,
                6..=9 => adaptive_compile_queue::CompilePriority::High,
                _ => adaptive_compile_queue::CompilePriority::Urgent,
            };
            // 添加到待编译队列（自适应队列自动处理优先级）
            let task = adaptive_compile_queue::CompileTask::new(pc, priority);
            self.pending_compile_queue.enqueue(task);
            // 发布热点检测事件
    ...
    pub fn process_compile_queue(&mut self, blocks: &HashMap<GuestAddr, IRBlock>) -> usize {
        ...
        // 处理队列中的高优先级块
        while let Some(task) = self.pending_compile_queue.dequeue() {
            // 检查时间预算
            ...
            // 查找对应的IR块
            if let Some(block) = blocks.get(&task.pc) {
                // 编译块
    ...
    pub fn prefetch_code_blocks(&mut self, current_pc: GuestAddr) {
        ...
        if let Some(ref collector) = self.profile_collector {
            ...
            if let Some(block_profile) = profile.block_profiles.get(&current_pc) {
                // 按调用频率排序被调用者 (使用keys的克隆列表)
                let mut callees: Vec<vm_core::GuestAddr> =
                    block_profile.callees.keys().cloned().collect();
                ...
                for &callee_pc in callees.iter().rev().take(3) {
                    // 检查是否已编译（AdaptiveCompileQueue 内部处理重复）
                    if self.cache.get(callee_pc).is_none() {
                        // 计算优先级（基于调用频率）
                        let exec_count = profile
                            .block_profiles
                            .get(&callee_pc)
                            .map(|p| p.execution_count.min(1000) / 10)
                            .unwrap_or(1);
                        let priority = if exec_count > 50 {
                            adaptive_compile_queue::CompilePriority::High
                        } else if exec_count > 10 {
                            adaptive_compile_queue::CompilePriority::Medium
                        } else {
                            adaptive_compile_queue::CompilePriority::Low
                        };
                        let task = adaptive_compile_queue::CompileTask::new(callee_pc, priority);
                        self.prefetch_compile_queue.enqueue(task);
                    }
                }
            }
        }
    ...
    pub fn process_prefetch_queue(&mut self, blocks: &HashMap<GuestAddr, IRBlock>) -> usize {
        ...
        // 处理预编译队列（限制数量，避免过度预取）
        while let Some(task) = self.prefetch_compile_queue.dequeue() {
            ...
            // 检查是否已编译
            if self.cache.get(task.pc).is_some() {
                continue;
            }

            // 查找对应的IR块并异步编译
            if let Some(block) = blocks.get(&task.pc).cloned() {
    ...
    pub fn add_to_compile_queue(&mut self, pc: GuestAddr, priority: u32) {
        // 转换为 CompilePriority
        let compile_priority = match priority {
            0..=1 => adaptive_compile_queue::CompilePriority::Low,
            2..=5 => adaptive_compile_queue::CompilePriority::Medium,
            6..=9 => adaptive_compile_queue::CompilePriority::High,
            _ => adaptive_compile_queue::CompilePriority::Urgent,
        };
        let task = adaptive_compile_queue::CompileTask::new(pc, compile_priority);
        self.pending_compile_queue.enqueue(task);
    }
    ...
    pub fn start_background_compile_task(&mut self) {
        ...
        // AdaptiveCompileQueue 内部已经使用 Arc，可以直接克隆引用
        let pending_queue = self.pending_compile_queue.clone();
        let prefetch_queue = self.prefetch_compile_queue.clone();
            ...
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        ...
                        // 处理pending队列（高优先级）
                        loop {
                            let task = pending_queue.dequeue();
                            ...
                            if let Some(block) = block {
                                ...
                                let handle = tokio::task::spawn_blocking(move || {
                                    ...
                                    // 存储结果
                                    results_clone.lock().insert(task_pc, code_ptr);
                                    cache_clone.insert(task_pc, code_ptr);
                                    tasks_clone.lock().remove(&task_pc);

                                    code_ptr
                                });

                                // JoinHandle已经实现了Send + Sync，可以直接存储
                                async_tasks.lock().insert(task.pc, Arc::new(handle));
                                compiled_count += 1;
                            }
                        ...
                        loop {
                            let task = prefetch_queue.dequeue();
                            ...
                            if prefetch_count >= 3 {
                                // 达到限制，将任务放回队列
                                prefetch_queue.enqueue(task);
                                break;
                            }

                            // 检查是否已编译
                            if cache.get(task.pc).is_some() {
                                continue;
                            }
    ...
    pub fn compile_async(&mut self, block: IRBlock) -> tokio::task::JoinHandle<CodePtr> {
            ...
            if let Some(existing_handle) = tasks.get(&pc) {
                ...
                if existing_handle.is_finished() {
                    ...
                    if let Some(ptr) = self.async_compile_results.lock().get(&pc).copied() {
                        // 返回一个立即完成的 future，直接返回已缓存的结果
                        let ptr_copy = ptr;
                        return tokio::spawn(async move { ptr_copy });
                    }
                } else {
        ...
        let mut loop_optimizer = self.loop_optimizer.clone();
        ...
        let handle = tokio::task::spawn_blocking(move || {
            ...
            // 允许循环优化器对 IR 进行预处理（如果提供）
            let mut block_for_compile = block_clone;
            loop_optimizer.optimize(&mut block_for_compile);

            // 如果热点计数显著低于阈值，跳过编译
            if let Some(hot) = hot_counts
                && hot.exec_count < adaptive_threshold_config.min_threshold
            {
                let null = CodeRef::null();
                results.lock().insert(pc, null);
                cache.insert(pc, null);
            ...
            // 使用 ML 指导（如果可用）进行决定
            #[cfg(feature = "experimental")]
            if let Some(mlc) = ml_compiler {
                let mut features =
                    ml_model::FeatureExtractor::extract_from_ir_block(&block_for_compile);
                if let Some(ref collector) = profile_collector {
                    let profile = collector.get_profile_data();
                    mlc.lock().enhance_features_with_pgo(
                        block_for_compile.start_pc,
                        &mut features,
                        &profile,
                    );
                }
                let decision = mlc
                    .lock()
                    .predict_decision(block_for_compile.start_pc, &features);
                if let ml_guided_jit::CompilationDecision::Skip = decision {
                    let null = CodeRef::null();
                    results.lock().insert(pc, null);
                    cache.insert(pc, null);
                    tasks.lock().remove(&pc);
            ...
            let code_ptr = temp_jit.compile(&block_for_compile);
    ...
    fn compile(&mut self, block: &IRBlock) -> CodePtr {
        ...
        let ml_decision = if let Some(ref ml_compiler) = self.ml_compiler {
            ...
            Some(
                ml_compiler
                    .lock()
                    .predict_decision(block.start_pc, &enhanced_features),
            )
        } else {
            None
        };
        #[cfg(not(feature = "experimental"))]
        let _ml_decision: Option<ml_stubs::CompilationDecision> = None;
        ...
        #[cfg(feature = "experimental")]
        if let Some(ml_guided_jit::CompilationDecision::Skip) = ml_decision {
            return CodeRef::null();
        }
        ...
        let use_fast_path = match ml_decision {
            Some(ml_guided_jit::CompilationDecision::FastJit) => true,
            Some(ml_guided_jit::CompilationDecision::OptimizedJit)
            | Some(ml_guided_jit::CompilationDecision::Aot) => false,
        ...
        let mut ctx = std::mem::replace(&mut self.ctx, CodegenContext::new());
        ...
        for op in &optimized_block.ops {
            match op {
                ...
                | IROp::VecMul {
                    ...
                    // 使用 element_size 做基本的断言，未来可以用来选择不同的向量化策略
                    debug_assert!(*element_size > 0, "element_size must be > 0");
                    let result = match op {
                    ...
                    // 使用 element_size 以便避免未使用警告并保留上下文语义
                    let _ = element_size;
        ...
        let code_ptr = CodeRef::from_ptr(code);
...
impl ExecutionEngine<IRBlock> for Jit {
    ...
    fn run(&mut self, mmu: &mut dyn MMU, block: &IRBlock) -> ExecResult {
            ...
            && ptr.is_null()
        {
            ...
            match &block.term {
                ...
                Terminator::JmpReg { base, offset } => {
                    ...
                    self.pc = vm_core::GuestAddr((base_val + *offset) as u64);
                }
                    ...
                    return make_result(
                        ExecStatus::Fault(vm_core::ExecutionError::Fault(
                            vm_core::Fault::PageFault {
                                addr: self.pc,
                                access_type: vm_core::AccessType::Execute,
                                is_write: false,
                                is_user: false,
                            },
                        )),
        ...
        if let Some(code_ptr) = code_ptr {
            ...
            let next = code_fn(&mut self.regs, &mut jit_ctx, &mut self.fregs);
            self.pc = vm_core::GuestAddr(next);
            ...
            if let Some(ref collector) = self.profile_collector {
                collector
                    .as_ref()
                    .record_block_execution(pc_key, exec_time_ns);
                // 记录调用关系
                if self.pc != vm_core::GuestAddr(0) && self.pc != pc_key {
                    collector.as_ref().record_block_call(self.pc, pc_key);
                }
            }
            // 记录ML训练样本（如果启用）
            #[cfg(feature = "experimental")]
            ...
            if let Some(ref collector) = self.profile_collector {
                collector
                    .as_ref()
                    .record_block_execution(pc_key, execution_time_ns);
                // 记录调用关系
                if self.pc != vm_core::GuestAddr(0) && self.pc != pc_key {
                    collector.as_ref().record_block_call(self.pc, pc_key);
                }
            ...
            match &block.term {
                ...
                Terminator::CondJmp {
                    ...
                    // 记录PGO数据：分支预测
                    if let Some(ref collector) = self.profile_collector {
                        collector.as_ref().record_branch(pc_key, target, taken);
                    }
                ...
                Terminator::JmpReg { base, offset } => {
                    let base_val = self.regs[*base as usize] as i64;
                    let target = vm_core::GuestAddr((base_val + *offset) as u64);

                    // 记录PGO数据：间接跳转
                    if let Some(ref collector) = self.profile_collector {
                        collector.as_ref().record_branch(pc_key, target, true);
                    }
                ...
                Terminator::Ret => {
                    ...
                    if let Some(ref collector) = self.profile_collector {
                        // 记录一次块执行作为返回事件的一部分
                        collector.as_ref().record_block_execution(pc_key, 0);
                    }
                    /* 保持当前 pc 以便上层处理 */
                }
                Terminator::Call { target, ret_pc: _ } => {
                    // 记录PGO数据：函数调用
                    if let Some(ref collector) = self.profile_collector {
                        collector.as_ref().record_function_call(
                            *target,
                            Some(pc_key),
                            execution_time_ns,
                        );
                        collector.as_ref().record_block_call(pc_key, *target);
                    }
                    self.pc = *target;
                }
                Terminator::Interrupt { .. } => { /* 保持当前 pc */ }
    ...
    fn get_vcpu_state(&self) -> vm_core::VcpuStateContainer {
        // Return a default container; detailed state sync will be implemented later
        vm_core::VcpuStateContainer::default()
    }
...
mod tests {
    ...
    fn test_jit_load_store_with_mmu() {
        ...
        mmu.write(vm_core::GuestAddr(0x200), 0xDEAD_BEEF_DEAD_BEEFu64, 8)
            .expect("Operation failed");
        ...
        assert_eq!(
            mmu.read(vm_core::GuestAddr(0x208), 2)
                .expect("Operation failed"),
            0xABCD
        );
    }
    ...
    fn test_jit_atomic_cas() {
        let mut mmu = SoftMmu::new(1024 * 1024, false);
        mmu.write(vm_core::GuestAddr(0x300), 0x1234_5678u64, 8)
            .expect("Operation failed");
        let mut ctx = JitContext { mmu: &mut mmu };
        let old = jit_cas(&mut ctx, 0x300, 0x1234_5678, 0xAAAA_BBBB, 8);
        assert_eq!(old, 0x1234_5678);
        assert_eq!(
            mmu.read(vm_core::GuestAddr(0x300), 8)
                .expect("Operation failed"),
            0xAAAA_BBBB
        );
    }
    ...
    fn test_ci_guard_jit_compiles() {
        ...
        let block = IRBlock {
            ...
            term: Terminator::Jmp {
                target: vm_core::GuestAddr(0x5000),
            },
        };
        jit.set_pc(block.start_pc);
        for _ in 0..HOT_THRESHOLD {
    ...
    fn test_jit_fload_fstore_consistency() {
        ...
        let block = IRBlock {
            ...
            term: Terminator::Jmp {
                target: vm_core::GuestAddr(0x6000),
            },
        };
        jit.set_pc(block.start_pc);
        for _ in 0..HOT_THRESHOLD {
        ...
        assert_eq!(jit.get_reg(5), bits_expected);
    }
