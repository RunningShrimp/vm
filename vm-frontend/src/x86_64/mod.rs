...
fn decode_insn_impl(
    ...
    let is_vex = prefix.rex.is_some() && matches!(prefix.rex, Some(0xC5 | 0xC4));
    let vex_w = if is_vex && prefix.rex == Some(0xC4) {
    ...
    if !is_evex && !is_vex {
        ...
        // Try AVX instruction type decoding
        if let Some(_avx_insn) = ExtendedDecoder::decode_avx_insn(&opcode_bytes, pc) {
            // For now, we'll handle AVX instructions through decode_avx
            // In a full implementation, we would map AVX instruction types to appropriate mnemonics
        }
            ...
            OpKind::Rm => {
                if mod_ == 3 {
                    ...
                    let final_base = if mod_ == 0
                        && (rm == 5 && !has_sib
                            || has_sib && (base.expect("Operation failed") & 7) == 5)
                    {
                        None
                    } else {
                        base
...
fn load_operand(builder: &mut IRBuilder, op: &X86Operand, op_bytes: u8) -> Result<u32, VmError> {
    match op {
        ...
        _ => Err(VmError::from(Fault::InvalidOpcode {
            pc: vm_core::GuestAddr(0),
            opcode: 0,
        })),
    }
...
fn translate_insn(builder: &mut IRBuilder, insn: X86Instruction) -> Result<(), VmError> {
    ...
    match insn.mnemonic {
        ...
        X86Mnemonic::Movntsd | X86Mnemonic::Movntss => {
            ...
            let flags = MemFlags {
                volatile: true, // Mark as non-temporal/volatile
                ..Default::default()
            };
        ...
        X86Mnemonic::Vsqrtps512 | X86Mnemonic::Vsqrtpd512 => {
            ...
            builder.push(IROp::FsqrtS {
                dst: result,
                src: result,
            }); // Use result as src for simplicity
            write_operand(builder, &insn.op1, result, 64)?;
        }
        ...
        X86Mnemonic::Vpermil2pd | X86Mnemonic::Vpermil2ps => {
            ...
            let src1 = load_operand(builder, &insn.op2, 16)?;
            let src2 = load_operand(builder, &insn.op3, 16)?;
            // Full implementation: read dest as control, then combine src1 and src2
            // For now, implement as blend based on control bits
            let temp = 400;
            builder.push(IROp::And {
                dst: temp,
                src1: dest,
        ...
        X86Mnemonic::Vpcmov => {
            ...
            // If mask bit is 1, copy from src1, else from src2
            let dest = load_operand(builder, &insn.op1, 16)?;
            let src1 = load_operand(builder, &insn.op2, 16)?;
            let src2 = load_operand(builder, &insn.op3, 16)?;
            // Implement as: result = (dest & src1) | (~dest & src2)
            let masked_src1 = 400;
            builder.push(IROp::And {
                dst: masked_src1,
                src1: dest,
    ...
    pub fn encode_mem64_ff(
        ...
        match (base, index) {
            ...
            _ => {
                ...
                let mod_bits = if force_disp32_only || disp == 0 {
                    0b00
                } else if disp8 as i32 == disp {
                    0b01
                } else {
