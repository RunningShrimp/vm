// vm-domain-events - 领域事件定义和处理逻辑
//! 定义虚拟机领域的核心事件，遵循DDD事件驱动架构原则。
//! 所有领域事件都是不可变的，表示已经发生的事实。

use vm_core::VmState;
#[allow(unused_imports)]
use vm_core::{GuestAddr, VmError, VmConfig};
use serde::{Deserialize, Serialize};
use std::time::SystemTime;

/// 领域事件基trait
///
/// 所有领域事件都应该实现这个trait，提供事件的基本元数据。
pub trait DomainEvent: Send + Sync {
    ...
    /// 是否支持版本升级
    fn supports_version_upgrade(&self) -> bool {
        false
    }
}
...
impl EventVersionMigrator {
    ...
    fn upgrade_event(event: DomainEventEnum, target_version: u64) -> Result<DomainEventEnum, String> {
        match event {
            ...
            DomainEventEnum::Memory(ref memory_event) => {
                Self::upgrade_memory_event(memory_event.clone(), target_version)
            }
            DomainEventEnum::Execution(ref execution_event) => {
                Self::upgrade_execution_event(execution_event.clone(), target_version)
            }
            DomainEventEnum::Device(ref device_event) => {
                Self::upgrade_device_event(device_event.clone(), target_version)
            }
            DomainEventEnum::Snapshot(_) => {
                // Snapshot events currently don't support versioning
                Ok(event)
            }
        }
    }
    ...
    fn upgrade_vm_lifecycle_event(event: VmLifecycleEvent, target_version: u64) -> Result<DomainEventEnum, String> {
        match target_version {
            2 => {
                ...
                Ok(DomainEventEnum::VmLifecycle(upgraded_event))
            }
            _ => Ok(DomainEventEnum::VmLifecycle(event)),
        }
    }
                ...
                let upgraded_event = match event {
                    MemoryEvent::MemoryAllocated { vm_id, addr, size, occurred_at } => {
                        MemoryEvent::MemoryAllocatedV2 {
                            ...
                            size,
                            occurred_at,
                            memory_type: "heap".to_string(), // 默认内存类型
                        }
                    }
                    // 其他事件保持不变
                    other => other,
                };
                Ok(DomainEventEnum::Memory(upgraded_event))
            }
            _ => Ok(DomainEventEnum::Memory(event)),
        }
    }
    ...
    /// 检查版本是否兼容
    pub fn is_version_compatible(event_type: &str, version: u64) -> bool {
        let latest = Self::get_latest_version(event_type);
        version <= latest
    }

    /// 创建版本迁移工具
    pub fn create_migration_tool() -> EventMigrationTool {
        EventMigrationTool::new()
    }
}
...
impl EventMigrationTool {
    ...
    /// 迁移单个事件
    pub fn migrate_event(&self, event: DomainEventEnum) -> Result<DomainEventEnum, String> {
        EventVersionMigrator::migrate_to_latest(event)
    }
    ...
    /// 验证事件版本兼容性
    pub fn validate_event_versions(&self, events: &[DomainEventEnum]) -> Vec<EventVersionIssue> {
        let mut issues = Vec::new();

        for event in events {
            let event_type = event.event_type();
            let version = event.version();
            let latest_version = EventVersionMigrator::get_latest_version(event_type);
...
pub struct EventVersionIssue {
    ...
    /// 问题类型
    pub issue_type: VersionIssueType,
    /// 问题描述
...
/// 事件升级报告
#[derive(Debug, Clone)]
pub struct EventUpgradeReport {
    /// 总事件数
    pub total_events: usize,
    /// 过时事件数
    pub outdated_events: usize,
    /// 可升级事件数
    pub upgradeable_events: usize,
    /// 不兼容事件数
    pub incompatible_events: usize,
    /// 具体问题列表
    pub issues: Vec<EventVersionIssue>,
}
...
impl DomainEvent for VmLifecycleEvent {
    ...
    fn occurred_at(&self) -> SystemTime {
        match self {
            VmLifecycleEvent::VmCreated { occurred_at, .. }
            | VmLifecycleEvent::VmCreatedV2 { occurred_at, .. }
            | VmLifecycleEvent::VmStarted { occurred_at, .. }
            | VmLifecycleEvent::VmStartedV2 { occurred_at, .. }
            | VmLifecycleEvent::VmPaused { occurred_at, .. }
            | VmLifecycleEvent::VmResumed { occurred_at, .. }
            | VmLifecycleEvent::VmStopped { occurred_at, .. }
            | VmLifecycleEvent::VmStoppedV2 { occurred_at, .. }
            | VmLifecycleEvent::VmStateChanged { occurred_at, .. } => *occurred_at,
        }
    }

    fn version(&self) -> u64 {
        match self {
            VmLifecycleEvent::VmCreated { .. } => 1,
            VmLifecycleEvent::VmCreatedV2 { .. } => 2,
            VmLifecycleEvent::VmStarted { .. } => 1,
            VmLifecycleEvent::VmStartedV2 { .. } => 2,
            VmLifecycleEvent::VmPaused { .. } => 1,
            VmLifecycleEvent::VmResumed { .. } => 1,
            VmLifecycleEvent::VmStopped { .. } => 1,
            VmLifecycleEvent::VmStoppedV2 { .. } => 2,
            VmLifecycleEvent::VmStateChanged { .. } => 1,
        }
    }
...
impl DomainEvent for MemoryEvent {
    fn event_type(&self) -> &'static str {
        match self {
            ...
            MemoryEvent::PageFault { .. } => "memory.page_fault",
        }
    }

    fn occurred_at(&self) -> SystemTime {
        match self {
            MemoryEvent::MemoryAllocated { occurred_at, .. }
            | MemoryEvent::MemoryAllocatedV2 { occurred_at, .. }
            | MemoryEvent::MemoryFreed { occurred_at, .. }
            | MemoryEvent::MemoryMapped { occurred_at, .. }
            | MemoryEvent::MemoryUnmapped { occurred_at, .. }
            | MemoryEvent::PageFault { occurred_at, .. } => *occurred_at,
        }
    }

    fn version(&self) -> u64 {
        match self {
            MemoryEvent::MemoryAllocated { .. } => 1,
            MemoryEvent::MemoryAllocatedV2 { .. } => 2,
            MemoryEvent::MemoryFreed { .. } => 1,
            MemoryEvent::MemoryMapped { .. } => 1,
            MemoryEvent::MemoryUnmapped { .. } => 1,
            MemoryEvent::PageFault { .. } => 1,
        }
    }

    fn supports_version_upgrade(&self) -> bool {
        matches!(self, MemoryEvent::MemoryAllocated { .. })
    }
}
...
impl DomainEvent for ExecutionEvent {
    fn event_type(&self) -> &'static str {
        match self {
            ...
            ExecutionEvent::VcpuExited { .. } => "execution.vcpu_exited",
    ...
    fn occurred_at(&self) -> SystemTime {
        match self {
            ...
            | ExecutionEvent::VcpuExited { occurred_at, .. }
            | ExecutionEvent::ExecutionStarted { occurred_at, .. }
            | ExecutionEvent::ExecutionStopped { occurred_at, .. } => *occurred_at,
        }
    }
}
...
impl DomainEvent for DeviceEvent {
    fn event_type(&self) -> &'static str {
        match self {
            ...
            DeviceEvent::DeviceInterrupt { .. } => "device.interrupt",
            DeviceEvent::DeviceIoCompleted { .. } => "device.io_completed",
        }
    }

    fn occurred_at(&self) -> SystemTime {
        match self {
            DeviceEvent::DeviceAdded { occurred_at, .. }
            | DeviceEvent::DeviceRemoved { occurred_at, .. }
            | DeviceEvent::DeviceInterrupt { occurred_at, .. }
            | DeviceEvent::DeviceIoCompleted { occurred_at, .. } => *occurred_at,
        }
    }
}
...
impl DomainEvent for SnapshotEvent {
    ...
    fn occurred_at(&self) -> SystemTime {
        match self {
            SnapshotEvent::SnapshotCreated { occurred_at, .. }
            | SnapshotEvent::SnapshotRestored { occurred_at, .. }
            | SnapshotEvent::SnapshotDeleted { occurred_at, .. } => *occurred_at,
        }
    }
}
...
impl DomainEvent for DomainEventEnum {
    fn event_type(&self) -> &'static str {
        match self {
            ...
            DomainEventEnum::Memory(e) => e.event_type(),
            DomainEventEnum::Execution(e) => e.event_type(),
            DomainEventEnum::Device(e) => e.event_type(),
            DomainEventEnum::Snapshot(e) => e.event_type(),
        }
    }
    ...
    fn occurred_at(&self) -> SystemTime {
        match self {
            ...
            DomainEventEnum::Memory(e) => e.occurred_at(),
            DomainEventEnum::Execution(e) => e.occurred_at(),
            DomainEventEnum::Device(e) => e.occurred_at(),
            DomainEventEnum::Snapshot(e) => e.occurred_at(),
        }
    }

    fn version(&self) -> u64 {
        match self {
            DomainEventEnum::VmLifecycle(e) => e.version(),
            DomainEventEnum::Memory(e) => e.version(),
            DomainEventEnum::Execution(e) => e.version(),
            DomainEventEnum::Device(e) => e.version(),
            DomainEventEnum::Snapshot(e) => e.version(),
        }
    }
}
...
impl From<&VmConfig> for VmConfigSnapshot {
...
mod tests {
    ...
    fn test_vm_lifecycle_event_types() {
        ...
        assert_eq!(created.event_type(), "vm.created");
        assert_eq!(created.occurred_at(), now);

        let started = VmLifecycleEvent::VmStarted {
            vm_id: "test-vm".to_string(),
            occurred_at: now,
        };
        assert_eq!(started.event_type(), "vm.started");

        let paused = VmLifecycleEvent::VmPaused {
            vm_id: "test-vm".to_string(),
            occurred_at: now,
        };
        assert_eq!(paused.event_type(), "vm.paused");

        let resumed = VmLifecycleEvent::VmResumed {
            vm_id: "test-vm".to_string(),
            occurred_at: now,
        };
        assert_eq!(resumed.event_type(), "vm.resumed");

        let stopped = VmLifecycleEvent::VmStopped {
            vm_id: "test-vm".to_string(),
            reason: "Test".to_string(),
            occurred_at: now,
        };
        assert_eq!(stopped.event_type(), "vm.stopped");
    ...
    fn test_memory_event_types() {
        ...
        assert_eq!(allocated.event_type(), "memory.allocated");
        assert_eq!(allocated.occurred_at(), now);
        ...
        assert_eq!(freed.event_type(), "memory.freed");
    }
    ...
    fn test_domain_event_enum() {
        ...
        assert_eq!(lifecycle.event_type(), "vm.started");
    ...
    #[test]
    fn test_vm_config_snapshot_from() {
        let config = VmConfig {
            guest_arch: GuestArch::Riscv64,
            memory_size: 128 * 1024 * 1024,
            vcpu_count: 4,
            exec_mode: vm_core::ExecMode::JIT,
            ..Default::default()
        };

        let snapshot = VmConfigSnapshot::from(&config);
        assert_eq!(snapshot.guest_arch, "riscv64");
        assert_eq!(snapshot.memory_size, 128 * 1024 * 1024);
        assert_eq!(snapshot.vcpu_count, 4);
        assert_eq!(snapshot.exec_mode, "JIT");
    }

    #[test]
    fn test_domain_event_version() {
        let now = SystemTime::now();
    ...
    #[test]
    fn test_event_version_upgrade() {
        let now = SystemTime::now();

        // 创建v1事件
        let v1_event = VmLifecycleEvent::VmCreated {
            vm_id: "test-vm".to_string(),
            config: VmConfigSnapshot {
                guest_arch: "riscv64".to_string(),
                memory_size: 64 * 1024 * 1024,
        ...
        // 升级到最新版本
        let domain_event = DomainEventEnum::VmLifecycle(v1_event);
        let upgraded = EventVersionMigrator::migrate_to_latest(domain_event).unwrap();
        ...
        match upgraded {
            ...
            _ => panic!("Expected VmCreatedV2"),
        }
    }

    #[test]
    fn test_memory_event_version_upgrade() {
        let now = SystemTime::now();
        ...
        // 升级到最新版本
        let domain_event = DomainEventEnum::Memory(v1_event);
        let upgraded = EventVersionMigrator::migrate_to_latest(domain_event).unwrap();
        ...
        match upgraded {
            ...
            _ => panic!("Expected MemoryAllocatedV2"),
        }
    }

    #[test]
    fn test_event_migration_tool() {
        let tool = EventVersionMigrator::create_migration_tool();
        let now = SystemTime::now();
        ...
        let events = vec![
            ...
            DomainEventEnum::VmLifecycle(VmLifecycleEvent::VmStartedV2 {
                vm_id: "vm1".to_string(),
                occurred_at: now,
                startup_time_ms: 100,
            }),
        ...
        // 生成升级报告
        let report = tool.generate_upgrade_report(&events);
        assert_eq!(report.total_events, 2);
        assert_eq!(report.outdated_events, 1); // VmCreated是v1，需要升级
        assert_eq!(report.upgradeable_events, 1);
        assert_eq!(report.incompatible_events, 0);

        // 验证问题详情
        assert_eq!(report.issues.len(), 1);
        assert_eq!(report.issues[0].event_type, "vm.created");
        assert_eq!(report.issues[0].current_version, 1);
        assert_eq!(report.issues[0].latest_version, 2);

        // 执行迁移
        let migrated = tool.migrate_events(events).unwrap();
    ...
    fn test_version_compatibility_check() {
        // 测试兼容版本
        assert!(EventVersionMigrator::is_version_compatible("vm.created", 1));
