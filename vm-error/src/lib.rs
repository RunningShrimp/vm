...
pub trait ErrorContextExt<T> {
    fn with_context(self, operation: impl Into<String>, component: impl Into<String>) -> VmResult<T>;
    fn with_file_context(self, file: impl Into<String>, line: u32) -> VmResult<T>;
}

impl<T> ErrorContextExt<T> for VmResult<T> {
    fn with_context(self, _operation: impl Into<String>, _component: impl Into<String>) -> VmResult<T> {
        self
    }
...
pub mod utils {
    ...
    pub fn is_recoverable(error: &VmError) -> bool {
        match error {
            VmError::Memory { source: MemoryError::PageFault(..), .. } => true,
            VmError::Network { source: NetworkError::Timeout(..), .. } => true,
            VmError::Io { .. } => true,
            _ => false,
        }
    }
    ...
    pub fn error_severity(error: &VmError) -> ErrorSeverity {
        match error {
            VmError::Core { source: CoreError::InvalidRegister(..), .. } => ErrorSeverity::Warning,
            VmError::Memory { source: MemoryError::AlignmentViolation(..), .. } => ErrorSeverity::Error,
            VmError::Translation { source: TranslationError::UnsupportedInstruction(..), .. } => ErrorSeverity::Error,
            VmError::JitCompilation { source: JitError::CodeGenerationFailed(..), .. } => ErrorSeverity::Critical,
            VmError::Device { source: DeviceError::InitializationFailed(..), .. } => ErrorSeverity::Error,
            VmError::Configuration { source: ConfigError::MissingRequired(..), .. } => ErrorSeverity::Error,
            VmError::Network { source: NetworkError::ConnectionFailed(..), .. } => ErrorSeverity::Error,
