...
impl fmt::Display for GpuError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GpuError::DeviceInitializationFailed { device_type, reason } => {
                write!(f, "Failed to initialize {} device: {}", device_type, reason)
            }
            GpuError::MemoryAllocationFailed { requested_size, reason } => {
                write!(
                    f,
            ...
            GpuError::KernelCompilationFailed { kernel_name, source, reason } => {
                write!(
                    f,
            ...
            GpuError::KernelLoadingFailed { kernel_name, reason } => {
            ...
            GpuError::FeatureNotSupported { feature, device } => {
                write!(f, "Feature '{}' not supported on device {}", feature, device)
            }
            GpuError::DriverBindingFailed { driver_type, reason } => {
