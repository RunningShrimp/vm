    ...
    fn read(&mut self, offset: u64, size: usize) -> Result<Vec<u8>, DeviceError> {
        match offset {
            ...
            0x01..=0x10 => {
                ...
                for (i, byte) in data.iter_mut().enumerate().take(size.min(6)) {
                    if offset + i as u64 >= 0x02 && (offset + i as u64) < 0x08 {
                        *byte = self.config.mac_address[i];
                    }
                }
                Ok(data)
            }
...
impl Device for VirtualDiskDevice {
    ...
    fn read(&mut self, offset: u64, size: usize) -> Result<Vec<u8>, DeviceError> {
        ...
        // Reset status back to Idle after synchronous read completes
        self.status = DeviceStatus::Idle;
    ...
    fn write(&mut self, offset: u64, data: &[u8]) -> Result<(), DeviceError> {
        ...
        // Reset status back to Idle after synchronous write completes
        self.status = DeviceStatus::Idle;
...
impl VirtualGpuDevice {
    ...
    pub fn write_pixel(&mut self, x: u32, y: u32, color: u32) {
        ...
        for (i, &byte) in color_bytes.iter().enumerate().take(bytes_per_pixel) {
...
impl Device for VirtualGpuDevice {
    ...
    fn io_notify(&mut self, event: IoEvent) {
        if let IoEvent::StatusChange { status } = event {
            self.status = status;
        }
    }
}
...
mod tests {
    ...
    fn test_device_manager() {
        ...
        let net_device =
            Box::new(VirtualNetworkDevice::new(NetworkConfig::default())) as Box<dyn Device>;
        let disk_device =
            Box::new(VirtualDiskDevice::new(DiskConfig::default())) as Box<dyn Device>;
