...
impl MockMMU {
    ...
    pub fn with_data(mut self, data: &[u8], offset: GuestAddr) -> Self {
        for (i, &byte) in data.iter().enumerate() {
            self.memory.insert(offset + i as u64, byte);
        }
        self
    }
}
...
impl MMU for MockMMU {
    ...
    fn dump_memory(&self) -> Vec<u8> {
        let max_addr = self.memory.keys().max().copied().unwrap_or(0);
        let mut result = vec![0u8; (max_addr + 1) as usize];
        for (&addr, &byte) in &self.memory {
            if addr < result.len() as u64 {
                result[addr as usize] = byte;
            }
        }
        result
    }
...
impl MMU for FaultyMMU {
    fn translate(&mut self, va: GuestAddr, access: AccessType) -> Result<GuestPhysAddr, VmError> {
        Err(vm_core::VmError::Execution(vm_core::ExecutionError::Fault(
            vm_core::Fault::PageFault { addr: va, access },
        )))
    }
...
mod tests {
    ...
    fn test_mock_mmu_basic() {
        ...
        mmu.write(0x100, 0x12345678, 4).unwrap();
        let val = mmu.read(0x100, 4).unwrap();
        assert_eq!(val, 0x12345678);
    }

    #[test]
    fn test_ir_block_builder() {
        let block = IRBlockBuilder::create_simple_arithmetic(0x1000);
