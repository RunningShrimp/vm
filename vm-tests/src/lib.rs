#[cfg(test)]
mod tests {
    use vm_core::{ExecutionEngine, MMU, Decoder};
    ...
    use vm_device::VirtioConsoleMmio;
    ...
    fn memorder_acquire_release_sequence_consistency() {
        let mut engine = Interpreter::new();
        let mut mmu = SoftMMU::new_with_size(0x3000);
        // addresses
    ...
    fn interpreter_atomic_minmax_combinations() {
        let mut engine = Interpreter::new();
        let mut mmu = SoftMMU::new_with_size(0x2000);
    ...
    fn interpreter_atomic_cmpxchg_flag_basic() {
        let mut engine = Interpreter::new();
        let mut mmu = SoftMMU::new_with_size(0x1000);
    ...
    fn paged_mmu_read_write_basic() {
        let mut mmu = vm_mem::PagedMMU::new(0x1000);
    ...
    fn vring_chain_used_updates() {
        let mut mmu = SoftMMU::new_with_size(0x2000);
        mmu.map_mmio(0x2000_0000, 0x1000, Box::new(VirtioConsoleMmio::new()));
    ...
    fn riscv_jal_decode_negative_extreme() {
        let mut mmu = SoftMMU::new_with_size(0x400000);
        let mut dec = RiscvDecoder;
    ...
    fn riscv_jal_decode_positive_extreme() {
        let mut mmu = SoftMMU::new_with_size(0x2000_000);
        let mut dec = RiscvDecoder;
    ...
    fn riscv_auipc_jalr_combo_executes() {
        let mut mmu = SoftMMU::new_with_size(0x10000);
        let mut dec = RiscvDecoder;
        let mut interp = Interpreter::new();
    ...
    fn vring_avail_event_default_and_feature_negotiation() {
        let mut mmu = SoftMMU::new_with_size(0x8000);
        mmu.map_mmio(0x2000_0000, 0x1000, Box::new(VirtioConsoleMmio::new()));
    ...
    fn vring_large_queue_event_index_boundary() {
        let mut mmu = SoftMMU::new_with_size(0x200000);
        mmu.map_mmio(0x2000_0000, 0x1000, Box::new(VirtioConsoleMmio::new()));
    ...
    impl MMU for FaultyMMU {
        fn translate(&mut self, _va: vm_core::GuestAddr, _access: vm_core::AccessType) -> Result<vm_core::GuestAddr, vm_core::Fault> {
            Err(vm_core::Fault::PageFault)
        }
