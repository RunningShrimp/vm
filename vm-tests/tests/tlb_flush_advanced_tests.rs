...
use vm_mem::tlb::tlb_flush_advanced::{
    AccessPredictor, AdaptiveFlushConfig, AdvancedTlbFlushConfig, AdvancedTlbFlushManager,
    PageImportanceEvaluator, PredictiveFlushConfig, SelectiveFlushConfig,
};
use vm_mem::tlb::tlb_flush::{FlushRequest, FlushScope, FlushStrategy, TlbFlushConfig};
use vm_mem::tlb::per_cpu_tlb::PerCpuTlbManager;
...
fn test_predictive_flush() {
    ...
    println!(
        "预测准确率: {:.2}%",
        predictive_stats.prediction_accuracy * 100.0
    );

    // 验证预测性刷新是否被触发
    assert!(
        predictive_stats.predictive_flushes > 0,
        "应该触发预测性刷新"
    );
}
...
fn test_selective_flush() {
    ...
    // 请求刷新包含热点和冷页面的范围
    let request = FlushRequest::new(FlushScope::PageRange, GuestAddr(hot_addr), GuestAddr(cold_addr), 0, 10, 0);
...
fn test_adaptive_flush() {
    ...
    for i in 0..30 {
        ...
        let request = FlushRequest::new(FlushScope::SinglePage, addr, addr, 0, 10, 0);
...
fn test_page_importance_evaluation() {
    ...
    // 验证评估结果
    assert!(
        frequent_importance > recent_importance,
        "频繁访问页面应该更重要"
    );
...
fn test_advanced_flush_manager_performance() {
    ...
    for i in 0..iterations {
        ...
        // 定期请求刷新
        if i % 10 == 0 {
            let request = FlushRequest::new(FlushScope::PageRange, addr, addr + 4096 * 5, 0, 10, 0);
    ...
    assert!(
        predictive_stats.prediction_accuracy > 0.5,
        "预测准确率应该超过50%"
    );
}
