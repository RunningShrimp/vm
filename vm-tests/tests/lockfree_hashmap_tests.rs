    ...
    for i in 0..thread_count / 2 {
        ...
        let handle = thread::spawn(move || {
            for j in 0..operations_per_thread {
                map.insert(i * operations_per_thread + j, i * operations_per_thread + j).unwrap();
            }
        });
...
fn test_high_concurrency() {
    ...
    // 验证统计信息
    assert_eq!(stats.insert_count, (thread_count / 2 * operations_per_thread) as usize);
    assert_eq!(stats.get_count, (thread_count / 2 * operations_per_thread) as usize);
}
...
fn test_hashmap_types_performance() {
    ...
    // 性能断言
    assert!(basic_ops > 100000.0, "基本无锁哈希表性能应该超过10万操作/秒");
