...
use vm_common::lockfree::{
    LockFreeSharedState, RwLockState, SharedStateStats, StateChange, StateChangeType, StateManager,
    StateSnapshot, StateSubscriber, StateVersion, StripedSharedState,
};
...
fn test_basic_lockfree_shared_state() {
    ...
    // 测试条件更新
    let conditional_snapshot = state.conditional_update(|x| x > 0, |x| x * 2);
    assert!(conditional_snapshot.is_some());
    assert_eq!(conditional_snapshot.unwrap().data, 2);

    // 测试条件更新失败
    let failed_snapshot = state.conditional_update(|x| x < 0, |x| x * 2);
...
fn test_state_manager() {
    ...
    // 测试条件更新状态
    let conditional_snapshot = manager.conditional_update(|x| x > 0, |x| x * 2);
    assert!(conditional_snapshot.is_some());
    assert_eq!(conditional_snapshot.unwrap().data, 2);
    ...
    for i in 0..thread_count {
        ...
        let handle = thread::spawn(move || {
            if i % 3 == 0 {
                ...
                // 更新线程
                for j in 0..operations_per_thread {
                    manager.update(|x| x + (i * operations_per_thread + j) as i32);
                }
            } else {
                // 条件更新线程 - 简化实现
                for j in 0..operations_per_thread {
...
fn test_state_change_notification() {
    ...
    manager.update(|x| x + 2);
    manager.replace_state(10);
    manager.update(|x| if x > 5 { x * 2 } else { x });
