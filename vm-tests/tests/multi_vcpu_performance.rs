...
use vm_core::{ExecutionEngine, MMU, ExecResult};
...
fn test_multi_vcpu_scaling() {
    ...
    for &vcpu_count in vcpu_counts.iter() {
        ...
        let blocks: Vec<vm_ir::IRBlock> = (0..vcpu_count)
            .map(|_| create_simple_ir_block(0x1000 + * as u64 * 0x100))
            .collect();
        ...
        // 验证结果
        assert_eq!(results.len(), vcpu_count as usize, "Should have results for all vCPUs");

        // 验证每个vCPU都成功执行
        for (i, result) in results.iter().enumerate() {
            assert!(matches!(result.status, vm_core::ExecStatus::Ok),
                   "vCPU {} failed execution: {:?}", i, result.status);
        }

        // 打印性能指标
        let total_ops: u64 = results.iter()
            .map(|r| r.stats.executed_insns)
            .sum();

        println!("  vCPUs: {}, Duration: {:?}, Total ops: {}, Ops/sec: {:.2}",
                vcpu_count, duration, total_ops,
                total_ops as f64 / duration.as_secs_f64());

        // 性能断言：多vCPU应该比单vCPU快（至少不慢太多）
        if vcpu_count > 1 {
            let ops_per_vcpu = total_ops as f64 / vcpu_count as f64;
            assert!(ops_per_vcpu > 1000.0,
...
fn test_load_balancing_strategies() {
    use vm_core::parallel::{VcpuLoadBalancer, LoadBalancePolicy};
    ...
    for policy in policies.iter() {
        ...
        if *policy == LoadBalancePolicy::RoundRobin {
            ...
            assert!(max_count - min_count <= 1, "RoundRobin should distribute evenly");
        }
    }
}
...
fn test_parallel_consistency() {
    ...
    for i in 0..vcpu_count {
        ...
        // 读取、修改、写回操作
        builder.push(IROp::Load { dst: 0, base: base_addr as u32, size: 8, offset: 0 });
        builder.push(IROp::Add { dst: 0, src1: 0, src2: 1 });
        builder.push(IROp::Store { base: base_addr as u32, src: 0, size: 8, offset: 0 });
...
fn test_many_vcpu_stress() {
    ...
    println!("Stress Test - {} vCPUs: Duration: {:?}, Total ops: {}, Ops/sec: {:.2}",
            large_vcpu_count, duration, total_ops,
            total_ops as f64 / duration.as_secs_f64());

    // 性能断言
    let ops_per_vcpu_per_sec = total_ops as f64 / (large_vcpu_count as f64) / duration.as_secs_f64();
    assert!(ops_per_vcpu_per_sec > 500.0, "Each vCPU should maintain at least 500 ops/sec under stress");
}