...
use vm_common::lockfree::{
    LockFreeQueue, BoundedLockFreeQueue, MpmcQueue, WorkStealingQueue,
    InstrumentedLockFreeQueue, QueueError,
};
...
fn test_high_concurrency() {
    ...
    // 验证统计信息
    assert_eq!(stats.push_count, (thread_count / 2 * operations_per_thread) as usize);
    assert_eq!(stats.pop_count, (thread_count / 2 * operations_per_thread) as usize);
}
