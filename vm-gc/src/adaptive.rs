...
use crate::generational::{
    GenerationalGCConfig, GenerationalGCStats as EnhancedGenerationalGCStats,
};
use crate::incremental::IncrementalGCStats;
...
impl PerformanceHistory {
    ...
    pub fn avg_fragmentation(&self) -> f64 {
        ...
        let sum: f64 = self
            .entries
            .iter()
            .map(|e| e.metrics.fragmentation_rate)
            .sum();
...
impl AdaptiveGCTuner {
    ...
    pub fn diagnose(&self, metrics: &GCPerformanceMetrics) -> GCProblem {
        ...
        if metrics.minor_gc_count > 0 {
            let promotion_rate = metrics.promoted_objects as f64 / metrics.collected_objects as f64;
    ...
    fn generate_reason(
        ...
        format!(
            "Detected: {}. Adjusted config: gc_threshold {:.2}->{:.2}, work_quota {}->{}, \
             nursery_ratio {:.2}->{:.2}",
            problem,
            old.gc_threshold,
            new.gc_threshold,
    ...
    pub fn compute_metrics_from_stats(
        ...
        heap_size: usize,
        used_memory: usize,
        stats: &EnhancedGenerationalGCStats,
        _incremental_stats: &IncrementalGCStats,
