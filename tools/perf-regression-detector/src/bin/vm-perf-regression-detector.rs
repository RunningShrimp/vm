...
use vm_perf_regression_detector::{
    collector::PerformanceCollector,
    config::RegressionDetectorConfig,
    detector::RegressionDetector,
    reporter::RegressionReporter,
    storage::{PerformanceStorage, SqlitePerformanceStorage},
};
    ...
    if collect_only {
        ...
        for i in 1..=iterations {
            ...
            println!(
                ...
                metrics.jit_compilation_time_us as f64 / 1000.0
            );
        ...
        let current_metrics = storage
            .get_latest_metrics(&context)?
        ...
        let critical_count = results
            .iter()
            .filter(|r| {
                r.severity == vm_perf_regression_detector::detector::RegressionSeverity::Critical
            })
            .count();
        let high_count = results
            .iter()
            .filter(|r| {
                r.severity == vm_perf_regression_detector::detector::RegressionSeverity::Major
            })
            .count();
        let medium_count = results
            .iter()
            .filter(|r| {
                r.severity == vm_perf_regression_detector::detector::RegressionSeverity::Moderate
            })
            .count();
        let low_count = results
            .iter()
            .filter(|r| {
                r.severity == vm_perf_regression_detector::detector::RegressionSeverity::Minor
            })
            .count();
