...
//!
//! CLI command to install Fedora from ISO with virtual disk

use std::path::{Path, PathBuf};
use vm_core::{ExecMode, GuestArch, VmConfig};
use vm_service::VmService;

/// Fedora Installer Command
pub struct FedoraInstallCommand {
    /// ISO path
    iso_path: PathBuf,
...
impl FedoraInstallCommand {
    /// Create new Fedora install command
    pub fn new<P: AsRef<Path>>(iso_path: P) -> Self {
        Self {
            iso_path: iso_path.as_ref().to_path_buf(),
            disk_path: None,
            disk_size_gb: 30, // Fedora needs similar space to Ubuntu
            memory_mb: 4096,  // 4GB for Fedora desktop
            vcpus: 1,
        }
    }
    ...
    /// Run Fedora installation
    pub async fn run(&self) -> Result<InstallResult, String> {
        println!("===========================================");
        println!("    Fedora Installation");
        println!("===========================================");
        println!();
        ...
        let kernel_path = "/tmp/fedora_iso_extracted/fedora_vmlinuz";
        ...
        println!("Starting Fedora boot sequence...");
        ...
        match service.boot_x86_kernel() {
            Ok(result) => {
                ...
                println!();
                println!("The Fedora installer interface should be visible");
                println!("above in the VGA Display Output section.");
                println!();
                println!("Fedora Installation Notes:");
                println!("  - Fedora requires 30GB minimum disk space");
                println!("  - 4GB RAM recommended for smooth installation");
                println!("  - Boot files loaded from Fedora ISO");
                println!();
                println!("If VGA display is empty:");
                println!("  - Fedora boot process may take longer");
    ...
    /// Extract kernel from Fedora ISO
    fn extract_kernel(&self) -> Result<(), String> {
        use std::process::Command;

        // Create extraction directory
        let extract_dir = "/tmp/fedora_iso_extracted";
        ...
        if use_7z {
            let kernel_dst = "/tmp/fedora_iso_extracted/fedora_vmlinuz";
            ...
            let extracted_path = format!("/tmp/fedora_iso_extracted/{}", extracted_name);

            if Path::new(&extracted_path).exists() && extracted_name != "fedora_vmlinuz" {
            ...
            // Extract kernel
            println!("  Extracting kernel files...");
            let kernel_src = "/tmp/fedora_iso_mounted/isolinux/vmlinuz";
            let kernel_dst = "/tmp/fedora_iso_extracted/fedora_vmlinuz";
            ...
            if !copy_result.status.success() {
                ...
                let alt_path = "/tmp/fedora_iso_mounted/images/pxeboot/vmlinuz";
                if Path::new(alt_path).exists() {
                    ...
                    let _ = Command::new("hdiutil")
                        .args(&["detach", "/tmp/fedora_iso_mounted"])
                        .output();
