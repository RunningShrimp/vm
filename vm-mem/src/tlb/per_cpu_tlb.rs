...
impl PerCpuTlbManager {
    ...
    fn get_current_cpu_id(&self) -> usize {
        ...
        {
            let mapping = self.thread_to_cpu.lock();
        ...
        {
            let mut mapping = self.thread_to_cpu.lock();
    ...
    pub fn lookup(&self, gva: GuestAddr, asid: u16) -> Option<GuestAddr> {
        ...
        let mut tlb = self.cpu_tlbs[cpu_id].lock();
    ...
    pub fn get_cpu_stats(&self, cpu_id: usize) -> Option<PerCpuTlbStatsSnapshot> {
        if cpu_id < self.cpu_tlbs.len() {
            let tlb = self.cpu_tlbs[cpu_id].lock();
            Some(tlb.get_stats())
        } else {
            None
        }
...
mod tests {
    ...
    fn test_per_cpu_tlb_entry() {
        let mut entry = PerCpuTlbEntry {
            gva: vm_core::GuestAddr(0x1000),
            gpa: vm_core::GuestAddr(0x2000),
            page_size: 4096,
    ...
    fn test_single_cpu_tlb() {
        ...
        // 查找条目
        let entry = tlb.lookup(vm_core::GuestAddr(0x1000), 0);
        assert!(entry.is_some());

        // 查找不存在的条目
        let entry = tlb.lookup(vm_core::GuestAddr(0x3000), 0);
    ...
    fn test_per_cpu_tlb_manager() {
        ...
        manager.insert(walk_result, vm_core::GuestAddr(0x1000), 0);

        // 查找条目
        let gpa = manager.lookup(vm_core::GuestAddr(0x1000), 0);
        assert_eq!(gpa, Some(vm_core::GuestAddr(0x2000)));

        // 查找不存在的条目
        let gpa = manager.lookup(vm_core::GuestAddr(0x3000), 0);
        assert_eq!(gpa, None);
