...
mod tests {
    ...
    fn test_small_copy() {
        ...
        let result = unsafe { copier.copy_memory(src.as_ptr(), dst.as_mut_ptr(), 10) };

        assert!(result.is_ok(), "Small copy should succeed: {:?}", result);
        assert_eq!(src, dst);
    ...
    fn test_large_copy() {
        ...
        let result = unsafe { copier.copy_memory(src.as_ptr(), dst.as_mut_ptr(), 10000) };

        assert!(result.is_ok(), "Large copy should succeed: {:?}", result);
        assert_eq!(src, dst);
