...
type TranslateFn = dyn Fn(GuestAddr, u16) -> Result<(GuestAddr, u64), VmError> + Send + Sync;

/// Type alias for read function
type ReadFn = dyn Fn(GuestAddr, usize) -> Result<Vec<u8>, VmError> + Send + Sync;

/// Type alias for write function
type WriteFn = dyn Fn(GuestAddr, &[u8]) -> Result<(), VmError> + Send + Sync;
