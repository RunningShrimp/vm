...
mod tests {
    ...
    fn test_memory_allocation_deallocation() {
        ...
        assert!(stats.local_allocs.load(std::sync::atomic::Ordering::Relaxed) > 0 ||
                stats.remote_allocs.load(std::sync::atomic::Ordering::Relaxed) > 0);
    ...
    fn test_allocation_failure_handling() {
        ...
        // 验证失败统计
        let stats = allocator.stats();
        let failed_count = stats.failed_allocs.load(std::sync::atomic::Ordering::Relaxed);
        // 失败计数可能为0或非0，取决于系统
        let _ = failed_count;
    }
...
mod benchmarks {
    ...
    fn benchmark_numa_allocation() {
        ...
        for &policy in &[NumaAllocPolicy::Local, NumaAllocPolicy::Interleave] {
            ...
            for &size in &config.allocation_sizes {
                ...
                println!("  Size {} bytes: {} ns/alloc ({} successful)",
                        size, avg_time as u64, successful_allocs);
            }

            // 输出统计信息
            let stats = allocator.stats();
            println!("  Local allocs: {}", stats.local_allocs.load(std::sync::atomic::Ordering::Relaxed));
            println!("  Remote allocs: {}", stats.remote_allocs.load(std::sync::atomic::Ordering::Relaxed));
            println!("  Failed allocs: {}", stats.failed_allocs.load(std::sync::atomic::Ordering::Relaxed));
        }
    }
    ...
    fn stress_test_memory_pressure() {
        ...
        // 输出最终统计
        let stats = allocator.stats();
        println!("Final stats - Local: {}, Remote: {}, Failed: {}",
...
mod gc_integration_tests {
    ...
    fn test_numa_memory_pressure_under_gc() {
        ...
        println!("  Local allocations: {}", stats.local_allocs.load(std::sync::atomic::Ordering::Relaxed));
        println!("  Remote allocations: {}", stats.remote_allocs.load(std::sync::atomic::Ordering::Relaxed));
        println!("  Failed allocations: {}", stats.failed_allocs.load(std::sync::atomic::Ordering::Relaxed));
    }
}
