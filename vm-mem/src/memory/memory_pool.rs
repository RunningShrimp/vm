...
impl<T: Default> MemoryPool<T> for StackPool<T> {
    ...
    fn shrink(&mut self) {
        // 保留当前使用量的1.5倍作为缓冲
        let keep_size = (self.stats.total_allocations - self.stats.total_deallocations) as usize * 3 / 2;
...
impl TlbEntryPool {
    ...
    fn get_next_pool_index(&self) -> usize {
        self.round_robin.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % self.pools.len()
    }
}
...
mod tests {
    ...
    fn test_tlb_entry_pool() {
        ...
        // 预分配
        pool.preallocate(200).unwrap();
        
        // 分配条目
        let entry1 = pool.allocate().unwrap();
        let entry2 = pool.allocate().unwrap();
