...
use crate::page_table_walker::{Sv39PageTableWalker, Sv48PageTableWalker};
use crate::tlb_concurrent::{ConcurrentTlbConfig, ConcurrentTlbManager, ConcurrentTlbManagerAdapter};
use crate::tlb_optimized::{MultiLevelTlb, MultiLevelTlbConfig};
...
use vm_core::{
    AccessType, GuestAddr, GuestPhysAddr, MMU, MemoryError as CoreMemoryError, MmioDevice, VmError,
};
...
impl MemoryPrefetcher {
    ...
    fn analyze_and_prefetch(&mut self, current_addr: GuestAddr) {
        ...
        let step = current_addr.wrapping_sub(last_addr);

        // 如果步长合理（在页面大小范围内），预测下一个地址
        if step > 0 && step <= PAGE_SIZE * 4 {
            let predicted_addr = current_addr.wrapping_add(step);
...
impl UnifiedMmu {
    ...
    pub fn new(size: usize, use_hugepages: bool, config: UnifiedMmuConfig) -> Self {
        ...
        let (multilevel_tlb, concurrent_tlb) = {
            ...
            (
                ...
                if use_concurrent {
                    ...
                    Some(Box::new(ConcurrentTlbManagerAdapter::new(concurrent_config)) as Box<dyn TlbManager>)
                } else {
                    None
                }
            )
        ...
        Self {
            ...
            config: config.clone(),
            stats: Arc::new(UnifiedMmuStats::new()),
    ...
    fn update_page_table_walker(&mut self) {
        self.page_table_walker = match self.paging_mode {
            PagingMode::Sv39 => Some(Box::new(Sv39PageTableWalker::new(self.page_table_base, self.asid))),
            PagingMode::Sv48 => Some(Box::new(Sv48PageTableWalker::new(self.page_table_base, self.asid))),
            _ => None,
        };
    }
    ...
    pub fn translate_with_cache(
        ...
        if let Some(entry) = tlb_result {
            let ppn = entry.phys_addr >> PAGE_SHIFT;
            ...
            if flags & required == 0 {
                return Err(VmError::from(vm_core::Fault::PageFault { addr: va, access }));
            }
            
            let offset = va & (PAGE_SIZE - 1);
            let pa = (ppn << PAGE_SHIFT) | offset;
            
            // 更新统计信息
            let elapsed = start_time.elapsed();
            self.stats.total_translation_time_ns.fetch_add(elapsed.as_nanos() as u64, Ordering::Relaxed);
        ...
        let (pa, flags) = match self.page_table_walker.take() {
            Some(mut walker) => {
                // 临时移出walker，避免双重借用
                let result = walker.walk(va, access, asid, self as &mut dyn MMU);
                // 将walker放回去
            ...
            None => {
                // 使用页表缓存或恒等映射
                let pa = self.page_table_walk_with_cache(va, access)?;
                Ok((pa, pte_flags::R | pte_flags::W | pte_flags::X))
            }
        }?;
        ...
        // 定期处理预取队列
        if self.config.enable_prefetch && self.stats.total_translations.load(Ordering::Relaxed) % 100 == 0 {
    ...
    fn process_prefetch_queue(&mut self) {
        ...
        for (vpn, asid) in prefetch_requests {
            ...
            let already_cached = match self.strategy {
                ...
                MmuOptimizationStrategy::Hybrid => {
                    let multilevel_cached = self.multilevel_tlb.as_mut()
                        .map(|tlb| tlb.lookup(va, asid, AccessType::Read).is_some())
                        .unwrap_or(false);
                    let concurrent_cached = self.concurrent_tlb.as_mut()
                        .map(|tlb| tlb.lookup(va, asid, AccessType::Read).is_some())
                        .unwrap_or(false);
    ...
    fn page_table_walk_with_cache(
        ...
        if let Some(ref cache) = self.page_table_cache {
            ...
            if let Some(pte_value) = cache_guard.lookup(self.page_table_base, level, vpn) {
                ...
                let pa = (ppn << PAGE_SHIFT) | (va & (PAGE_SIZE - 1));
                return Ok(pa);
            } else {
                self.stats
        ...
        // 缓存缺失，使用页表遍历器或恒等映射
        if let Some(ref walker) = self.page_table_walker {
            ...
            if let Some(ref cache) = self.page_table_cache {
                ...
                let vpn = va >> PAGE_SHIFT;
                let level = 0;
                let pte_value = (pa >> PAGE_SHIFT) << 10; // 简化PTE构造
    ...
    fn check_alignment(&self, pa: GuestAddr, size: u8) -> Result<(), VmError> {
        if self.strict_align {
            match size {
                1 => {}
                2 => if pa % 2 != 0 { return Err(VmError::from(vm_core::Fault::AlignmentFault { addr: pa, size })); },
                4 => if pa % 4 != 0 { return Err(VmError::from(vm_core::Fault::AlignmentFault { addr: pa, size })); },
                8 => if pa % 8 != 0 { return Err(VmError::from(vm_core::Fault::AlignmentFault { addr: pa, size })); },
                _ => return Err(VmError::from(vm_core::Fault::AlignmentFault { addr: pa, size })),
            }
        } else if !matches!(size, 1|2|4|8) {
            return Err(VmError::from(vm_core::Fault::AlignmentFault { addr: pa, size }));
        }
        Ok(())
    }
    ...
    fn read_phys(&self, pa: GuestPhysAddr, size: u8) -> Result<u64, VmError> {
        ...
        match size {
            ...
            _ => return Err(VmError::from(vm_core::Fault::AlignmentFault { addr: pa, size })),
        }.map_err(|_| VmError::from(vm_core::Fault::AccessViolation { addr: pa, access: AccessType::Read }))
    }
    ...
    fn write_phys(&mut self, pa: GuestPhysAddr, val: u64, size: u8) -> Result<(), VmError> {
        ...
        if res.is_err() {
            return Err(VmError::from(vm_core::Fault::AccessViolation { addr: pa, access: AccessType::Write }));
        }
...
impl MMU for UnifiedMmu {
    ...
    fn memory_size(&self) -> usize {
        self.phys_mem.total_size
    }

    fn dump_memory(&self) -> Vec<u8> {
        let mut buf = vec![0u8; self.phys_mem.total_size];
        let _ = self.phys_mem.read_buf(0, &mut buf);
        buf
    }

    fn restore_memory(&mut self, data: &[u8]) -> Result<(), String> {
        if data.len() != self.phys_mem.total_size {
            return Err(format!(
        ...
        self.phys_mem.write_buf(0, data).map_err(|e| {
            format!("Failed to restore memory: {:?}", e)
        })
    }
