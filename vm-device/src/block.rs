...
#[repr(u8)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum VirtioEvent { Notify = 0, Wake = 16, IndexMatch = 32 }
...
impl VirtioBlock {
                ...
                match req {
                    BlockIoRequest::Read { sector, count, req_id } => {
                        ...
                        match backend_file.read_exact(&mut buf).await {
                            Ok(_) => { let _ = resp_tx.send(BlockIoResponse::ReadOk { data: buf, req_id }).await; }
                            Err(e) => { let _ = resp_tx.send(BlockIoResponse::Error { req_id, msg: e.to_string() }).await; }
                        }
                    }
                    BlockIoRequest::Write { sector, data, req_id } => {
                        ...
                        match backend_file.write_all(&data).await {
                            Ok(_) => { let _ = resp_tx.send(BlockIoResponse::WriteOk { req_id }).await; }
                            Err(e) => { let _ = resp_tx.send(BlockIoResponse::Error { req_id, msg: e.to_string() }).await; }
                        }
                    }
                    BlockIoRequest::Flush { req_id } => {
                        match backend_file.sync_all().await {
                            Ok(_) => { let _ = resp_tx.send(BlockIoResponse::FlushOk { req_id }).await; }
                            Err(e) => { let _ = resp_tx.send(BlockIoResponse::Error { req_id, msg: e.to_string() }).await; }
                        }
                    }
                }
    ...
    pub fn features(&self) -> u32 {
        ...
        if self.read_only { features |= 1 << features::VIRTIO_BLK_F_RO; }
        features
    }
    ...
    pub fn handle_request(
        ...
        let req_type = match mmu.read(req_addr, 4) { Ok(v) => v as u32, Err(_) => return BlockStatus::IoErr };
        let sector = match mmu.read(req_addr + 8, 8) { Ok(v) => v, Err(_) => return BlockStatus::IoErr };
        let req_type = match BlockRequestType::from_u32(req_type) { Some(t) => t, None => return BlockStatus::Unsupported };
    ...
    fn handle_read(
        ...
        self.next_req_id += 1;
        if let Err(_) = self.tx.blocking_send(BlockIoRequest::Read { sector, count: data_len, req_id }) {
        ...
        match rx.blocking_recv() {
            Some(BlockIoResponse::ReadOk { data, req_id: resp_id }) => {
                if resp_id != req_id { return BlockStatus::IoErr; }
                if let Some(sm) = mmu.as_any_mut().downcast_mut::<SoftMmu>() {
                    if let Err(_) = sm.write_bulk(data_addr, &data) { return BlockStatus::IoErr; }
                } else if let Err(_) = MmuUtil::write_slice(mmu, data_addr, &data) { return BlockStatus::IoErr; }
                BlockStatus::Ok
            }
            _ => BlockStatus::IoErr,
            _ => BlockStatus::IoErr,
    ...
    fn handle_write(
        ...
        if self.read_only { return BlockStatus::IoErr; }

        let mut buffer = vec![0u8; data_len as usize];
        if let Some(sm) = mmu.as_any().downcast_ref::<SoftMmu>() {
            if let Some(slice) = sm.guest_slice(data_addr, data_len as usize) { buffer.copy_from_slice(&slice); } else { return BlockStatus::IoErr; }
        } else if let Err(_) = MmuUtil::read_slice(mmu, data_addr, &mut buffer) { return BlockStatus::IoErr; }

        let req_id = self.next_req_id;
        self.next_req_id += 1;
        if let Err(_) = self.tx.blocking_send(BlockIoRequest::Write { sector, data: buffer, req_id }) { return BlockStatus::IoErr; }

        let mut rx = self.rx.lock().expect("Failed to lock receiver");
        match rx.blocking_recv() {
            Some(BlockIoResponse::WriteOk { req_id: resp_id }) => { if resp_id != req_id { return BlockStatus::IoErr; } BlockStatus::Ok }
            _ => BlockStatus::IoErr,
        }
    }
    ...
    fn handle_flush(&mut self) -> BlockStatus {
        let req_id = self.next_req_id;
        self.next_req_id += 1;
        if let Err(_) = self.tx.blocking_send(BlockIoRequest::Flush { req_id }) { return BlockStatus::IoErr; }

        let mut rx = self.rx.lock().expect("Failed to lock receiver");
        match rx.blocking_recv() {
            Some(BlockIoResponse::FlushOk { req_id: resp_id }) => { if resp_id != req_id { return BlockStatus::IoErr; } BlockStatus::Ok }
            _ => BlockStatus::IoErr,
        }
    }
}
...
impl VirtioBlockMmio {
    ...
    /// 设备事件枚举
    fn set_event(&mut self, q: u32, ev: VirtioEvent) { self.cause_evt |= 1u64 << (ev as u64 + q as u64); }
    ...
    pub fn handle_queue_notify(&mut self, mmu: &mut dyn MMU, _queue_idx: u32) {
        if self.avail_addr == 0 || self.desc_addr == 0 || self.used_addr == 0 { return; }

        let avail_idx = match mmu.read(self.avail_addr + 2, 2) { Ok(v) => v as u16, Err(_) => return };
        ...
        while self.used_idx.wrapping_add(processed) != avail_idx {
            ...
            let desc_idx = match mmu.read(desc_idx_addr, 2) { Ok(v) => v as u16, Err(_) => break };
        ...
        self.set_event(q, VirtioEvent::Notify);
        self.set_event(q, VirtioEvent::IndexMatch);
        self.set_event(q, VirtioEvent::Wake);
        let q = self.selected_queue;
        self.set_event(q, VirtioEvent::Notify);
        self.set_event(q, VirtioEvent::IndexMatch);
        self.set_event(q, VirtioEvent::Wake);
    ...
    fn process_descriptor_chain(&mut self, mmu: &mut dyn MMU, desc_idx: u16) {
        ...
        loop {
            ...
            if len == 1 && (flags & 0x2) != 0 { status_addr = Some(addr); } else { total_len = total_len.wrapping_add(len); }
...
impl vm_core::MmioDevice for VirtioBlockMmio {
    fn read(&self, offset: u64, size: u8) -> u64 { self.read(offset, size) }
    fn write(&mut self, offset: u64, val: u64, size: u8) { self.write(offset, val, size); }
    fn notify(&mut self, mmu: &mut dyn vm_core::MMU, _offset: u64) { self.handle_queue_notify(mmu, self.selected_queue); }
}
