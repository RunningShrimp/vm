...
impl VirtioCrypto {
    ...
    /// Helper to acquire next_request_id lock with error handling
    fn lock_next_request_id(&self) -> VmResult<std::sync::MutexGuard<'_, u64>> {
        self.next_request_id.lock().map_err(|_| {
            VmError::Memory(MemoryError::PageTableError {
                message: "VirtioCrypto next_request_id lock is poisoned".to_string(),
                level: None,
            })
        })
    }
    ...
    fn allocate_request_id(&self) -> u64 {
        match self.lock_next_request_id() {
            Ok(mut next) => {
                let id = *next;
