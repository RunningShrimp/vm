...
use vm_core::{VmError, VmResult, VmState};
use vm_core::vm_state::VirtualMachineState;
use std::sync::{Arc, Mutex};

/// 启动虚拟机
pub fn start<B>(state: Arc<Mutex<VirtualMachineState<B>>>) -> VmResult<()> {
        ...
        if state_guard.state() != VmState::Created && state_guard.state() != VmState::Paused {
        ...
        state_guard.set_state(VmState::Running);
        Ok(())
}

/// 暂停虚拟机
pub fn pause<B>(state: Arc<Mutex<VirtualMachineState<B>>>) -> VmResult<()> {
        let mut state_guard = state.lock().map_err(|_| {
            VmError::Core(vm_core::CoreError::Internal {
                message: "Failed to lock state".to_string(),
                module: "LifecycleManager".to_string(),
        ...
        if state_guard.state() != VmState::Running {
            return Err(VmError::Core(vm_core::CoreError::Config {
        ...
        state_guard.set_state(VmState::Paused);
        Ok(())
}

/// 停止虚拟机
pub fn stop<B>(state: Arc<Mutex<VirtualMachineState<B>>>) -> VmResult<()> {
        let mut state_guard = state.lock().map_err(|_| {
            VmError::Core(vm_core::CoreError::Internal {
                message: "Failed to lock state".to_string(),
                module: "LifecycleManager".to_string(),
        ...
        state_guard.set_state(VmState::Stopped);
        Ok(())
}

/// 重置虚拟机
pub fn reset<B>(state: Arc<Mutex<VirtualMachineState<B>>>) -> VmResult<()> {
        let mut state_guard = state.lock().map_err(|_| {
            VmError::Core(vm_core::CoreError::Internal {
                message: "Failed to lock state".to_string(),
                module: "LifecycleManager".to_string(),
        ...
        state_guard.set_state(VmState::Created);
