...
//                 path: Some("config.memory_size".to_string()),
//             }));
//         }

//         // 业务逻辑：验证虚拟机状态
//         // 注意：is_running 方法不可用，改为检查状态字段
//         if state.state() == vm_core::VmState::Running {
//             return Err(VmError::Core(vm_core::CoreError::Config {
...
//         let execution_handler = ExecutionEventHandler::new(vm_id_str.clone(), event_bus.clone());
...
//                 })
//             })?;
//             // VirtualMachineState 没有 Clone 实现，创建一个新的
//             // 注意：这里需要检查VirtualMachineState::new的参数类型
//             // 错误：expected `Box<dyn MMU>`, found `Arc<Mutex<Box<dyn MMU>>>`
//             // 所以我们创建一个新的MMU而不是克隆
...
//         vm_id_str: &str,
//     ) -> VmResult<()> {
//         // 基础设施：设置事件存储处理器 - 注释掉，因为存在类型不匹配问题
//         // 错误：EventStore 不是 Fn(&dyn DomainEvent) -> Result<(), VmError> + Send + Sync
//         // 此功能需要重新设计以符合 DomainEventBus API
//         // let event_store_handler = SimpleEventHandler::new(
//         //     move |event: &dyn DomainEvent| {
//         //         let event_clone = event.clone(); // 需要 DomainEvent 有 Clone 实现
//         //         // 这里应该实现事件存储逻辑
//         //         Ok(())
//             }
//         // );
...
//     }
    ...
    //         self.event_store
    //             .append(&vm_id_str, Some(last_seq + 1), event)?;
    //     }
    ...
    //     // 从事件重建聚合
    //     let new_aggregate =
    //         VirtualMachineAggregate::from_events(vm_id_str.to_string(), config, events);
    ...
    //     info!(
    //         "Replayed {} events for VM {}",
    //         self.event_store.get_event_count(&vm_id_str)?,
    //         vm_id_str
    //     );
    //     Ok(())
    // }
