...
use super::gc_incremental_enhanced::ObjectPtr;
...
impl GenerationalGC {
    ...
    pub fn new(heap_size: usize, config: GenerationalGCConfig) -> Self {
        let mut card_table = CardTable::new(heap_size, config.card_size);
    ...
    pub fn minor_gc(&mut self) {
        ...
        // 2. 处理脏卡片（老年代到新生代的引用）
        let dirty_objects: Vec<ObjectPtr> = self.card_table
            .dirty_cards()
    ...
    fn trace_all_generations(&mut self) {
        // 1. 收集新生代对象
        let nursery_objects: Vec<ObjectPtr> = self.nursery
            .iter()
            .map(|(obj, _)| *obj)
            .collect();

        // 2. 收集老年代对象
        let old_gen_objects: Vec<ObjectPtr> = self.old_gen
            .iter()
            .map(|(obj, _)| *obj)
            .collect();
    ...
    fn compact_old_gen(&mut self) {
        ...
        // 这里简化为保留活跃对象
        self.old_gen
            .retain(|(_, metadata)| metadata.age < 255); // 移除过老的对象
    }
...
mod tests {
    ...
    fn test_promotion_age() {
        ...
        // 第一次Minor GC
        gc.minor_gc();
        println!("After 1st minor_gc: old_gen.len()={}, nursery.len()={}", gc.old_gen.len(), gc.nursery.len());
        assert_eq!(gc.old_gen.len(), 0);

        // 第二次Minor GC (应该已经经历了2次GC)
        let _keep_alive = obj1;  // 保持obj1活着
        gc.minor_gc();
        println!("After 2nd minor_gc: old_gen.len()={}, nursery.len()={}", gc.old_gen.len(), gc.nursery.len());
    ...
    #[test]
    fn test_nursery_usage() {
        let config = GenerationalGCConfig {
            nursery_size: 16384, // 16KB
            ..Default::default()
        };
        let mut gc = GenerationalGC::new(1024 * 1024, config);

        // 初始使用率
        assert_eq!(gc.nursery_usage(), 0.0);

        // 分配8KB
        gc.alloc(8192).unwrap();

        // 使用率应该约50%
        let usage = gc.nursery_usage();
    ...
    fn test_stats() {
        ...
        stats.minor_gc_count.store(5, Ordering::Relaxed);
        stats.minor_gc_time_ns.store(10_000_000, Ordering::Relaxed); // 10ms

        assert_eq!(stats.avg_minor_gc_time_ms(), 2.0);
    }

    #[test]
    fn test_needs_major_gc() {
        let config = GenerationalGCConfig::default();
        let gc = GenerationalGC::new(1024 * 1024, config);

        // 初始不需要Major GC
        assert!(!gc.needs_major_gc());

        // 模拟10次Minor GC
        gc.stats.minor_gc_count.store(10, Ordering::Relaxed);
