...
use crate::{
    Alert, DashboardConfig, MetricsCollector, PerformanceSnapshot, alerts::AlertManager,
...
impl DashboardServer {
    ...
    async fn create_router(&self) -> Result<Router, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let performance_analyzer =
            Arc::new(PerformanceAnalyzer::new(self.metrics_collector.clone()));
        ...
        let app = Router::new()
            ...
            .route(
                ...
                get(performance_report_json_handler),
            )
            .route(
                "/api/performance/report/text",
                get(performance_report_text_handler),
            )
    ...
    /// 推送指标到WebSocket
    #[allow(dead_code)] // 该方法为future实现保留
    async fn push_websocket_metrics(
        &self,
...
async fn metrics_handler(
    State((metrics_collector, alert_manager, _)): State<(
