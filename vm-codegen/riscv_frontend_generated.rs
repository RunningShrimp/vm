//! # vm-frontend-risc-v - RISC-V 前端解码器
//!
//! 提供 RISC-V 架构的指令解码器，将 RISC-V 机器码转换为 VM IR。
//!
//! ## 使用示例
//!
//! ```rust,ignore
//! use vm_frontend_risc-v::RISC-VDecoder;
//!
//! let mut decoder = RISC-VDecoder::new();
...
impl Default for RISC-VDecoder {
    fn default() -> Self {
        Self::new()
    }
}
impl RISC-VDecoder {
    /// 解码单条指令
    pub fn decode_insn(&mut self, insn: u32, pc: u64) -> RISC-VInstruction {
        ...
        RISC-VInstruction::new(mnemonic, pc + 4, has_memory_op, is_branch)
    }
    
    /// 解码指令块
    pub fn decode_block(&mut self, data: &[u8], pc: u64) -> Result<Vec<RISC-VInstruction>, String> {
