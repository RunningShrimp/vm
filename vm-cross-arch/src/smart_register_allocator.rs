...
impl SmartRegisterMapper {
    ...
    pub fn new(target_arch: Architecture) -> Self {
        ...
        let temp_pool: Vec<RegId> = calling_conv.caller_saved
            .iter()
    ...
    /// 构建寄存器冲突图
    pub fn build_interference_graph(&self, live_ranges: &[(RegId, (usize, usize))]) -> Vec<InterferenceNode> {
    ...
    /// 使用图着色算法分配寄存器
    pub fn allocate_registers(&mut self, live_ranges: &[(RegId, (usize, usize))]) -> Result<(), String> {
...
mod tests {
    ...
    fn test_interference_graph() {
        ...
        // 验证冲突关系
        let node0 = nodes.iter().find(|n| n.reg == 0).unwrap();
        let node1 = nodes.iter().find(|n| n.reg == 1).unwrap();
        let node2 = nodes.iter().find(|n| n.reg == 2).unwrap();
