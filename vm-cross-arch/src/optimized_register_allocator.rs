...
use super::{Architecture, CallingConvention, InterferenceNode, RegisterClass, RegisterInfo};
...
impl OptimizedRegisterMapper {
    ...
    pub fn analyze_lifetimes(&mut self, instructions: &[vm_ir::IROp]) {
        ...
        // 创建生命周期记录 - 包含所有被定义的寄存器
        let all_regs: HashSet<RegId> = use_counts
            .keys()
            .copied()
        ...
        for &reg in &all_regs {
            ...
            let last_use = last_uses.get(&reg).copied().unwrap_or(def_point); // 如果未使用，last_use等于def点
    ...
    pub fn eliminate_copies(&mut self, instructions: &[vm_ir::IROp]) -> Vec<vm_ir::IROp> {
        ...
        for (idx, insn) in instructions.iter().enumerate() {
            if let vm_ir::IROp::Mov { dst, src } = insn {
                // 记录拷贝
    ...
    /// 检查是否可以消除寄存器拷贝
    fn can_eliminate_copy(
        &self,
        src: RegId,
        dst: RegId,
        copy_map: &HashMap<(RegId, RegId), usize>,
    ) -> bool {
        // 如果源和目标相同，可以消除
        if src == dst {
            return true;
