...
impl TargetSpecificOptimizer {
    ...
    pub fn detect_and_analyze_loops(&mut self, ops: &[IROp]) -> Vec<IROp> {
        ...
        for (idx, op) in ops.iter().enumerate() {
            match op {
                // 查找所有可能的向后跳转分支指令
                IROp::Beq { src1: _, src2: _, target: _ } |
                IROp::Bne { src1: _, src2: _, target: _ } |
                IROp::Blt { src1: _, src2: _, target: _ } |
                IROp::Bge { src1: _, src2: _, target: _ } |
                IROp::Bltu { src1: _, src2: _, target: _ } |
                IROp::Bgeu { src1: _, src2: _, target: _ } => {
                    if idx > 0 { // 确保不是第一条指令
                        loop_count += 1;
                    }
                },
