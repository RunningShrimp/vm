...
use vm_core::encoding::{
    EncodingContext, EncodingError, ImmediateFormat, InstructionBuilder, InstructionFlag,
    RegisterField, utils,
};
use vm_error::Architecture as VmErrorArchitecture;
use vm_ir::instruction_patterns::DefaultPatternMatcher;
use tracing;
use vm_ir::RegId;
use vm_core::memory_access::{
    AccessType, AccessWidth, Alignment, DefaultMemoryAccessOptimizer, MemoryAccessOptimizer,
    MemoryAccessPattern,
};
use vm_engine_jit::optimization::{
    CommonSubexpressionEliminationPass, ConstantFoldingPass, DeadCodeEliminationPass,
    OptimizationContext, OptimizationLevel, OptimizationPipeline,
};
use vm_core::register::{MappingStrategy, RegisterClass, RegisterMapper, RegisterSet};
...
impl RefactoredArchEncoder {
    ...
    fn add_x86_registers(register_set: &mut RegisterSet) {
        use vm_core::register::{RegisterInfo, RegisterType};
    ...
    fn encode_arithmetic_op(
        ...
        let target_src1 = self
            .register_mapper
            .map_register(src1)
            .map_err(|_| EncodingError::InvalidFormat)?;
        let target_src2 = self
            .register_mapper
            .map_register(src2)
            .map_err(|_| EncodingError::InvalidFormat)?;
        ...
        match self.architecture {
            ...
            Architecture::ARM64 => {
                ...
                let word = base_opcode
                    | (target_dst & 0x1F)      // Rd
                    | ((target_src1 & 0x1F) << 5) // Rn
                    | ((target_src2 & 0x1F) << 16); // Rm
            ...
            Architecture::RISCV64 => {
                ...
                let word = base_opcode
                    | ((target_dst & 0x1F) << 7)   // rd
                    | ((target_src1 & 0x1F) << 15)  // rs1
                    | ((target_src2 & 0x1F) << 20); // rs2
        ...
        match self.architecture {
            ...
            Architecture::ARM64 => {
                ...
                let base_opcode = match (access_type, effective_width) {
                    ...
                    (AccessType::Read, AccessWidth::DoubleWord | AccessWidth::QuadWord) => {
                        0xF9400000
                    } // LDR X[t], [X[n]]
                    ...
                    (AccessType::Write, AccessWidth::DoubleWord | AccessWidth::QuadWord) => {
                        0xF9000000
                    } // STR X[t], [X[n]]
...
pub fn example_refactored_encoding() -> Result<(), EncodingError> {
    ...
    tracing::debug!("ADD instruction encoded: {:?}", add_instructions);
    ...
    tracing::debug!("LOAD instruction encoded: {:?}", load_instructions);
...
mod tests {
    ...
    fn test_arithmetic_encoding() {
        let mut encoder = RefactoredArchEncoder::new(Architecture::X86_64).unwrap();
    ...
    fn test_memory_encoding() {
        let mut encoder = RefactoredArchEncoder::new(Architecture::X86_64).unwrap();
        let result = encoder.encode_memory_op(3, 4, 0x1000, AccessWidth::Word, AccessType::Read);
        assert!(result.is_ok());
