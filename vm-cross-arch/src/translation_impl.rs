...
use crate::encoder::ArchEncoder;
use crate::instruction_parallelism::{InstructionParallelizer, ResourceRequirements};
use crate::optimized_register_allocator::OptimizedRegisterMapper;
use crate::smart_register_allocator::SmartRegisterMapper;
use crate::types::TranslationError;
...
impl ArchTranslator {
    ...
    pub fn get_optimization_stats(
        &self,
    ) -> Option<&super::optimized_register_allocator::OptimizationStats> {
    ...
    pub fn get_ir_optimization_stats(&self) -> Option<IROptimizationStats> {
        self.ir_optimizer
            .as_ref()
            .map(|optimizer| optimizer.get_stats().clone())
    }
    ...
    fn analyze_live_ranges(&self, block: &vm_ir::IRBlock) -> Vec<(vm_ir::RegId, (usize, usize))> {
        let mut def_points: std::collections::HashMap<vm_ir::RegId, usize> =
            std::collections::HashMap::new();
        let mut use_points: std::collections::HashMap<vm_ir::RegId, usize> =
            std::collections::HashMap::new();
    ...
    fn encode_jump(
        ...
        let offset = (target.0.wrapping_sub(pc.0.wrapping_add(4))) as i64;
    ...
    fn encode_conditional_jump(
        ...
        let offset_true = (target_true.0.wrapping_sub(pc.0.wrapping_add(4))) as i64;
        let offset_false = (target_false.0.wrapping_sub(pc.0.wrapping_add(4))) as i64;
...
pub struct CrossArchBlockCache {
    _max_size: usize,
    _replacement_policy: CacheReplacementPolicy,
}
...
impl CrossArchBlockCache {
    ...
    pub fn set_max_size(&self, _size: usize) {}
}
...
pub struct MemoryAlignmentOptimizer {
    _source_endianness: Endianness,
    _target_endianness: Endianness,
    _conversion_strategy: EndiannessConversionStrategy,
}
...
impl MemoryAlignmentOptimizer {
    pub fn new(
        source: Endianness,
        target: Endianness,
        strategy: EndiannessConversionStrategy,
    ) -> Self {
        Self {
            _source_endianness: source,
            _target_endianness: target,
            _conversion_strategy: strategy,
        }
    }
...
impl AdaptiveOptimizer {
    ...
    pub fn optimize(&mut self, ops: &[vm_ir::IROp], _pc: u64) -> Vec<vm_ir::IROp> {
        ops.to_vec()
    }
