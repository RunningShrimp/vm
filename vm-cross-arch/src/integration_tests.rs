...
fn test_cross_arch_execution_basic() {
    ...
    let mut runtime = CrossArchRuntime::new(config, 1024 * 1024)
...
fn test_cross_arch_jit_compilation() {
    ...
    assert!(exec_result.is_ok(), "Execution should succeed after compilation");
}
...
fn test_cross_arch_aot_loading() {
    ...
    let runtime = CrossArchRuntime::new(config, 1024 * 1024)
        .expect("Failed to create runtime with AOT");
...
fn test_cross_arch_error_handling() {
    ...
    assert!(result.is_err() || result.is_ok(), "Error handling should work");
}
    ...
    let handles: Vec<_> = (0..4)
        .map(|_| {
            ...
            std::thread::spawn(move || {
                CrossArchRuntime::new(config_clone, 1024 * 1024)
            })
        })
        .collect();
