...
use crate::architecture::DistributedArchitectureConfig;
use crate::discovery::{VmDiscovery, VmInfo};
use crate::protocol::{TaskId, TaskInfo, TaskStatus, TaskType, VmMessage};
...
impl VmCoordinator {
    ...
    async fn select_round_robin(&self, active_vms: &[VmInfo]) -> VmInfo {
        let mut index = self.next_vm_index.lock().unwrap();
