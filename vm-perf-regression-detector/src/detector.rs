...
use super::config::{RegressionDetectorConfig, DetectionAlgorithm, MetricThreshold};
...
impl RegressionDetector {
    ...
    pub fn detect_regressions(
        ...
        if let Some(threshold) = self.config.metric_thresholds.get("execution_time") {
            if threshold.enabled {
                if let Some(result) = self.detect_execution_time_regression(
                    current_metrics,
                    historical_metrics,
                    threshold,
                )? {
        ...
        if let Some(threshold) = self.config.metric_thresholds.get("memory_usage") {
            if threshold.enabled {
                if let Some(result) = self.detect_memory_usage_regression(
                    current_metrics,
                    historical_metrics,
                    threshold,
                )? {
        ...
        if let Some(threshold) = self.config.metric_thresholds.get("jit_compilation_time") {
            if threshold.enabled {
                if let Some(result) = self.detect_jit_compilation_time_regression(
                    current_metrics,
                    historical_metrics,
                    threshold,
                )? {
        ...
        if let Some(threshold) = self.config.metric_thresholds.get("instruction_throughput") {
            if threshold.enabled {
                if let Some(result) = self.detect_instruction_throughput_regression(
                    current_metrics,
                    historical_metrics,
                    threshold,
                )? {
            ...
            if let Some(threshold) = self.config.metric_thresholds.get(metric_name) {
                if threshold.enabled {
                    ...
                    if historical_values.len() >= threshold.min_samples {
                        if let Some(result) = self.detect_custom_metric_regression(
                            metric_name,
                            *value,
                            &historical_values,
                            threshold,
    ...
    fn detect_regression(
        ...
        let (p_value, algorithm_name) = match &self.config.detection_config.algorithm {
            ...
            DetectionAlgorithm::MovingAverage { window_size } => {
                let p_value = self.moving_average_test(current_value, historical_values, *window_size)?;
                (Some(p_value), format!("Moving Average (window={})", window_size))
            },
    ...
    /// 移动平均检测
    fn moving_average_test(&self, current_value: f64, historical_values: &[f64], window_size: usize) -> Result<f64> {
...
mod tests {
    use super::*;
    use crate::config::{MetricThreshold, DetectionAlgorithm};
    ...
    fn test_regression_detection() -> Result<()> {
        ...
        let historical_metrics: Vec<PerformanceMetrics> = historical_values
            ...
            .map(|(i, &exec_time)| PerformanceMetrics {
                ...
                cache_hit_rate: 0.95,
                custom_metrics: HashMap::new(),
                timestamp: chrono::Utc::now() - chrono::Duration::days((historical_values.len() - i) as i64),
            })
            .collect();
        ...
        let exec_time_regression = results.iter()
