...
use super::config::{ReportFormat, ReportConfig};
...
impl RegressionReporter {
    ...
    fn generate_text_report(&self, results: &[RegressionResult]) -> Result<String> {
        ...
        let critical_count = results.iter().filter(|r| r.severity == RegressionSeverity::Critical).count();
        let major_count = results.iter().filter(|r| r.severity == RegressionSeverity::Major).count();
        let moderate_count = results.iter().filter(|r| r.severity == RegressionSeverity::Moderate).count();
        let minor_count = results.iter().filter(|r| r.severity == RegressionSeverity::Minor).count();
    ...
    fn generate_html_report(&self, results: &[RegressionResult]) -> Result<String> {
        ...
        report.push_str("        table { border-collapse: collapse; width: 100%; margin-top: 20px; }\n");
        ...
        if !results.is_empty() {
            ...
            for result in results {
                ...
                report.push_str(&format!("            <td>{:.2}</td>\n", result.current_value));
                report.push_str(&format!("            <td>{:.2}</td>\n", result.baseline_value));
                report.push_str(&format!("            <td>{:.2}%</td>\n", result.percentage_change));
                report.push_str(&format!("            <td class=\"{}\">{:?}</td>\n", severity_class, result.severity));
    ...
    fn generate_markdown_report(&self, results: &[RegressionResult]) -> Result<String> {
        ...
        if !results.is_empty() {
            report.push_str("## 回归详情\n\n");
            report.push_str("| 指标 | 当前值 | 基准值 | 变化 | 严重程度 | 统计显著性 | 检测算法 | 检测时间 |\n");
            report.push_str("|------|--------|--------|------|----------|------------|----------|----------|\n");
    ...
    fn generate_charts(&self, results: &[RegressionResult]) -> Result<()> {
        ...
        // 按严重程度分组
        let mut grouped_results: HashMap<RegressionSeverity, Vec<&RegressionResult>> = HashMap::new();
        for result in results {
            let entry = grouped_results.entry(result.severity.clone()).or_insert_with(Vec::new);
            entry.push(result);
        }
    ...
    /// 生成严重程度分布图
    fn generate_severity_chart(&self, grouped_results: &HashMap<RegressionSeverity, Vec<&RegressionResult>>) -> Result<()> {
        ...
        let data = [
            ("关键", grouped_results.get(&RegressionSeverity::Critical).map_or(0, |v| v.len()) as f32),
            ("严重", grouped_results.get(&RegressionSeverity::Major).map_or(0, |v| v.len()) as f32),
            ("中等", grouped_results.get(&RegressionSeverity::Moderate).map_or(0, |v| v.len()) as f32),
            ("轻微", grouped_results.get(&RegressionSeverity::Minor).map_or(0, |v| v.len()) as f32),
            ("无", grouped_results.get(&RegressionSeverity::None).map_or(0, |v| v.len()) as f32),
        ];
        ...
        let max_count = data.iter()
            .map(|&(_, count)| count)
            .fold(0.0, |max, current| if current > max { current } else { max }) * 1.2;
        ...
        for (i, &(label, _)) in data.iter().enumerate() {
            ...
            chart.draw_series(
                vec![Text::new(label.to_string(), (x, -5.0), ("Arial", 12).into_font())]
            )?;
        }
    ...
    fn generate_metrics_change_chart(&self, results: &[RegressionResult]) -> Result<()> {
        ...
        for result in results {
            let entry = grouped_metrics.entry(result.metric_name.clone()).or_insert_with(Vec::new);
            entry.push(result);
        }
        ...
        for (metric_name, metric_results) in grouped_metrics {
            ...
            // 计算数据范围
            let min_y = data.iter().map(|&(_, y)| y).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(0.0);
            let max_y = data.iter().map(|&(_, y)| y).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(100.0);
            ...
            let mut chart = ChartBuilder::on(&chart_root)
                ...
                .y_label_area_size(30)
                .build_cartesian_2d(
                    0.0..(metric_results.len() as f32 - 1.0),
                    lower_y..upper_y
                )?;
            ...
            // 绘制数据点
            chart.draw_series(data.iter().map(|&(x, y)| Circle::new((x, y), 2, RED.filled())))?;
        }
