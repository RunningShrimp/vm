...
impl InstructionDecoder for X86_64Decoder {
    fn decode(&self, bytes: &[u8]) -> LiftResult<(Instruction, usize)> {
        if bytes.is_empty() {
            return Err(LiftError::DecodeError("Empty instruction bytes".to_string()).into());
        }
...
fn decode_x86_64_manual(bytes: &[u8]) -> LiftResult<Instruction> {
    if bytes.is_empty() {
        return Err(LiftError::DecodeError("Empty bytes".to_string()).into());
    }
    ...
    if offset >= bytes.len() {
        return Err(LiftError::DecodeError("Incomplete instruction".to_string()).into());
    }
    ...
    match opcode {
        ...
        0x89 => {
            ...
            if offset >= bytes.len() {
                return Err(
                    LiftError::DecodeError("Incomplete MOV instruction".to_string()).into(),
                );
            }
        ...
        0x01 => {
            ...
            if offset >= bytes.len() {
                return Err(
                    LiftError::DecodeError("Incomplete ADD instruction".to_string()).into(),
                );
            }
            let modrm = bytes[offset];
            let (dst, src, _) = decode_modrm_reg_reg(modrm, rex);
        ...
        0x29 => {
            ...
            if offset >= bytes.len() {
                return Err(
                    LiftError::DecodeError("Incomplete SUB instruction".to_string()).into(),
                );
            }
            let modrm = bytes[offset];
            let (dst, src, _) = decode_modrm_reg_reg(modrm, rex);
        ...
        0xFF => {
            // CALL/JMP indirect
            if offset >= bytes.len() {
                return Err(LiftError::DecodeError("Incomplete FF instruction".to_string()).into());
            }
            let modrm = bytes[offset];
            ...
            match reg_opcode {
                ...
                _ => Err(
                    LiftError::DecodeError(format!("Unsupported FF variant: {}", reg_opcode))
                        .into(),
                ),
            }
        }
        _ => Err(
            LiftError::DecodeError(format!("Unsupported x86-64 opcode: 0x{:02X}", opcode)).into(),
        ),
    }
}
...
impl InstructionDecoder for ARM64Decoder {
    fn decode(&self, bytes: &[u8]) -> LiftResult<(Instruction, usize)> {
        if bytes.len() < 4 {
            return Err(
                LiftError::DecodeError("ARM64 instruction must be 4 bytes".to_string()).into(),
            );
        }
...
fn decode_arm64_manual(word: u32) -> LiftResult<Instruction> {
    ...
    match top_bits {
        ...
        _ => Err(
            LiftError::DecodeError(format!("Unsupported ARM64 instruction: 0x{:08X}", word)).into(),
        ),
    }
}
...
impl InstructionDecoder for RISCV64Decoder {
    fn decode(&self, bytes: &[u8]) -> LiftResult<(Instruction, usize)> {
        ...
        // 简化：都当作 4 字节
        if bytes.len() < 4 {
            return Err(LiftError::DecodeError("Incomplete RISC-V instruction".to_string()).into());
        }
...
fn decode_riscv_manual(word: u32) -> LiftResult<Instruction> {
    ...
    match opcode {
        ...
        _ => Err(
            LiftError::DecodeError(format!("Unsupported RISC-V opcode: 0x{:02X}", opcode)).into(),
        ),
    }
}
